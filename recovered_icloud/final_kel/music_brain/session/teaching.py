...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "challenges":
            # Launch challenge system
            from .interactive_challenges import ChallengeSystem
            system = ChallengeSystem()
            if len(sys.argv) > 2:
                system.interactive_challenge_session(sys.argv[2])
            else:
        ...
        # Show menu with challenges option
        print("\n" + "=" * 60)
        print("🎸 DAiW RULE-BREAKING TEACHING MODULE")
        print("=" * 60)
        print("\nOptions:")
        print("  [L]essons - Interactive lessons on rule-breaking")
        print("  [C]hallenges - Hands-on challenges to practice")
        print("  [E]xit")
