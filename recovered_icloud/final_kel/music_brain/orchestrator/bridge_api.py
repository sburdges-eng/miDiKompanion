...
# =============================================================================
# C++ ORCHESTRATOR INTEGRATION FUNCTIONS
# =============================================================================

# Global orchestrator instance for C++ integration
...
def execute_pipeline_sync(
    ...
    """
    Execute a Python orchestrator pipeline synchronously.

    This function is designed to be called from C++ via Python bridge.

    Args:
        pipeline_name: Name of pipeline to execute (e.g., "idaw_bridge", "full_generation")
    ...
    try:
        ...
        pipeline = _create_pipeline(pipeline_name)

        # Execute pipeline synchronously
        loop = asyncio.new_event_loop()
        try:
            result = loop.run_until_complete(
                _orchestrator_instance.execute(pipeline, input_data)
            )
...
def execute_pipeline_async(
    ...
    """
    Execute pipeline asynchronously (returns immediately with execution ID).

    Args:
        pipeline_name: Name of pipeline
        input_data_json: Input data JSON

    Returns:
        JSON string with execution ID:
        {
            "execution_id": "uuid",
            "status": "running"
        }
    ...
    # Execute in background (simplified - in production, use proper async execution)
...
def _create_pipeline(pipeline_name: str) -> Pipeline:
    """
    Create a pipeline based on name.
    ...
    if pipeline_name == "idaw_bridge":
        # Standard iDAW bridge pipeline
        pipeline.add_stage("intent", IntentProcessor())
        pipeline.add_stage("harmony", HarmonyProcessor())
        pipeline.add_stage("groove", GrooveProcessor())
    elif pipeline_name == "full_generation":
        # Full generation pipeline with all stages
        pipeline.add_stage("intent", IntentProcessor())
        pipeline.add_stage("harmony", HarmonyProcessor())
        pipeline.add_stage("groove", GrooveProcessor())
...
# =============================================================================
# ORCHESTRATOR BRIDGE FUNCTIONS (for OrchestratorBridge C++ class)
# =============================================================================

def execute_pipeline_sync(pipeline_name: str, input_data_json: str) -> str:
    """
    Execute an orchestrator pipeline synchronously.

    Used by C++ OrchestratorBridge.
    ...
    try:
        ...
        # Add default stages if pipeline is empty
        ...
        # Execute pipeline
        result = asyncio.run(orchestrator.execute(pipeline, input_data))
        ...
        result_dict = {
            "success": True,
            "chords": result.context.get_shared("harmony", {}).get("chords", []) if result.context else [],
            "melody": result.context.get_shared("melody", []) if result.context else [],
            "bass": result.context.get_shared("bass", []) if result.context else [],
            "metadata": result.context.get_shared("metadata", {}) if result.context else {},
        }
...
def pipeline_exists(pipeline_name: str) -> bool:
    ...
    """
    # For now, we'll check if it's a known pipeline name
    # In a full implementation, this would check a registry
    known_pipelines = ["idaw_bridge", "full_generation", "intent_only"]
    return pipeline_name in known_pipelines
...
def get_available_pipelines() -> List[str]:
    ...
    """
    # For now, return known pipelines
    # In a full implementation, this would query a registry
    return ["idaw_bridge", "full_generation", "intent_only"]
