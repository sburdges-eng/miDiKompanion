...
class GrooveProcessor(BaseProcessor):
    ...
    async def _generate_groove(self, input_data: GrooveInput) -> GrooveOutput:
        ...
        # Try learned groove profile override if available
        try:
            manager = GrooveLearningManager()
            profile = manager.load_profile("default")
            if profile:
                learned = manager.generate(
                ...
                return GrooveOutput(
                    pattern_name=learned.get("pattern_name", groove.pattern_name),
                    tempo_bpm=learned.get("tempo_bpm", groove.tempo_bpm),
                    swing_factor=learned.get("swing_factor", groove.swing_factor),
                    timing_offsets_16th=learned.get("timing_offsets_16th", groove.timing_offsets_16th),
                    velocity_curve=learned.get("velocity_curve", groove.velocity_curve),
