...
class IntentProcessor(BaseProcessor):
    ...
    async def _process_intent(self, input_data: IntentInput) -> IntentOutput:
        ...
        # Apply learned rule-break preferences if available
        try:
            from music_brain.learning.rulebreak_learning import RuleBreakLearningManager
            rb_manager = RuleBreakLearningManager()
            learned_rule = rb_manager.choose(
                emotion=input_data.mood_primary,
                profile="default"
            )
            if learned_rule and learned_rule not in suggested_rules:
                suggested_rules = suggested_rules or []
                suggested_rules.append(learned_rule)
        except Exception:
            pass
