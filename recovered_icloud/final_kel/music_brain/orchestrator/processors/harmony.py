...
class HarmonyProcessor(BaseProcessor):
    ...
    async def _generate_harmony(self, input_data: HarmonyInput) -> HarmonyOutput:
        ...
        # Try learned harmony profile as an override if available
        try:
            manager = HarmonyLearningManager()
            profile = manager.load_profile("default")
            if profile:
                learned_chords = manager.generate(
                    emotion=input_data.emotion,
                    profile_name="default",
                    length=input_data.progression_length,
                    key=input_data.key,
                    mode=input_data.mode,
                )
                # Build a simple HarmonyOutput from learned chords
                return HarmonyOutput(
                    chords=learned_chords,
                    roman_numerals=["?"] * len(learned_chords),
                    key=input_data.key,
                    mode=input_data.mode,
                    rule_broken=input_data.rule_to_break,
                    emotional_arc=[input_data.emotion] * len(learned_chords),
                    voice_leading_notes=[],
                )
        except Exception:
            # If learning not available, fall back to existing path
            pass
