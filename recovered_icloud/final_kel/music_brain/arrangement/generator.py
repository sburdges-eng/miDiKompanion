...
class ArrangementGenerator:
    ...
    def generate(
        ...
        # Try learned arrangement profile override
        try:
            manager = ArrangementLearningManager()
            profile = manager.load_profile("default")
            if profile:
                learned = manager.generate_arrangement(primary_emotion, "default", genre=genre)
                # Replace sections/instruments if learned data exists
                if learned.get("sections"):
                    template.sections = [
                        SectionTemplate(
                            name=s,
                            type=SectionType.CHORUS if "chorus" in s.lower() else SectionType.VERSE
                        )
                        for s in learned["sections"]
                    ]
                learned_instruments = learned.get("instruments") or []
            else:
                learned_instruments = []
        except Exception:
            learned_instruments = []
        ...
        if learned_instruments:
            instruments = [
                InstrumentTrack(name=name, midi_channel=0, entry_section=0, exit_section=None, velocity_curve=[])
                for name in learned_instruments
            ]
