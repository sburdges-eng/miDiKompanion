...
def generate_bass_line(
    ...
    # Try learned bass profile override if available
    try:
        manager = BassLearningManager()
        profile = manager.load_profile("default")
        if profile:
            learned_notes = manager.generate(
                emotion="neutral",
                profile_name="default",
                length=len(chord_progression) * 2  # rough length estimate
            )
            # Map learned notes into BassNote objects (quarter-note spacing)
            notes_objs = []
            tick = 0
            ticks_per_note = ppq
            for n in learned_notes:
                notes_objs.append(BassNote(pitch=n, start_tick=tick, duration_ticks=ticks_per_note, velocity=80))
                tick += ticks_per_note
            return BassLine(notes=notes_objs, pattern=pattern, octave=octave)
    except Exception:
        pass
