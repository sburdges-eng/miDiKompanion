---
title: "Week of <% tp.date.now('YYYY-MM-DD', 0, tp.file.title, 'YYYY-[W]WW') %>"
created: <% tp.date.now("YYYY-MM-DD") %>
type: weekly-review
week: <% tp.date.now("YYYY-[W]WW") %>
tags:
  - type/weekly-review
---

# Weekly Review — <% tp.date.now("YYYY-[W]WW") %>

**Week of:** <% tp.date.now("MMMM Do") %> to <% tp.date.now("MMMM Do", 6) %>

---

## Quick Stats

| Metric | This Week | Goal |
|--------|-----------|------|
| Songs worked on | | |
| Recording sessions | | |
| Practice time (min) | | |
| Mixes completed | | |
| New ideas captured | | |

---

## What I Accomplished

### Songs/Projects
- [ ]
- [ ]
- [ ]

### Learning
- [ ]
- [ ]

### Administrative
- [ ]
- [ ]

---

## Creative Wins 🎉

*What went well? What am I proud of?*

1.
2.
3.

---

## Challenges Faced

*What was difficult? What didn't go as planned?*

1. **Challenge:**
   **How I handled it:**
  
2. **Challenge:**
   **How I handled it:**

---

## Ideas Generated

*Capture any creative ideas that came up this week*

### Song Ideas
-
-

### Sound/Production Ideas
-
-

### Process Improvements
-
-

---

## What I Learned

### Technical Skills
-

### Creative Insights
-

### About My Process
-

---

## Projects Status Update

### Active Projects
| Project | Status | Progress | Next Step |
|---------|--------|----------|-----------|
| [[]] | | | |
| [[]] | | | |
| [[]] | | | |

### Completed This Week
-

### Started This Week
-

### On Hold
-

---

## Time Allocation

### How I Spent My Music Time
| Activity | Hours | Notes |
|----------|-------|-------|
| Writing/Composing | | |
| Recording | | |
| Mixing | | |
| Practice | | |
| Learning | | |
| Admin/Organization | | |
| **Total** | | |

### Was this balanced?
- [ ] Yes, good allocation
- [ ] Too much on:
- [ ] Not enough on:

---

## Review of Goals

### Last Week's Goals
| Goal | Status | Notes |
|------|--------|-------|
| | ✅ / ⬜ / 🔄 | |
| | | |
| | | |

### What helped me achieve goals?
-

### What got in the way?
-

---

## Next Week's Focus

### Top 3 Priorities
1.
2.
3.

### Specific Goals
- [ ]
- [ ]
- [ ]

### What I'm Excited About
-

### Potential Obstacles
-
- **How I'll handle it:**

---

## Session Planning

### Scheduled Time
| Day | Time | Focus |
|-----|------|-------|
| Monday | | |
| Tuesday | | |
| Wednesday | | |
| Thursday | | |
| Friday | | |
| Saturday | | |
| Sunday | | |

---

## Gear/Tools Notes

### Something that worked great
-

### Something that frustrated me
-

### Want to try/buy
-

---

## Inspiration & References

### Tracks that inspired me this week
1.
2.

### Articles/videos worth revisiting
-

---

## Energy & Motivation

### How I felt this week
- **Energy level:** Low / Medium / High
- **Motivation:** Low / Medium / High
- **Creative flow:** Blocked / Sporadic / Flowing

### What affected my energy?
- **Positive:**
- **Negative:**

### Self-care check
- [ ] Getting enough sleep
- [ ] Taking breaks
- [ ] Protecting my hearing
- [ ] Maintaining work-life balance

---

## Gratitude

*Three things I'm grateful for in my music journey this week:*

1.
2.
3.

---

## Notes



---

## Related
- [[Week of <% tp.date.now("YYYY-MM-DD", -7) %>]]
- [[Songs/]]
- [[Projects/]]

