---
title: "Practice Log - <% tp.date.now('YYYY-MM-DD') %>"
created: <% tp.date.now("YYYY-MM-DD") %>
type: practice
date: <% tp.date.now("YYYY-MM-DD") %>
duration_minutes:
instrument:
focus:
tags:
  - type/practice
---

# Practice Log — <% tp.date.now("dddd, MMMM Do") %>

## Session Overview

| Property | Value |
|----------|-------|
| **Date** | <% tp.date.now("YYYY-MM-DD") %> |
| **Duration** | <% tp.file.cursor() %> minutes |
| **Instrument** | Guitar / Keys / Vocals / Production / Other |
| **Energy Level** | Low / Medium / High |
| **Focus Level** | ⭐⭐⭐⭐⭐ |

---

## Today's Focus

### Main Goal
*What am I trying to improve today?*



### Specific Exercises
- [ ]
- [ ]
- [ ]

---

## Warm-Up (10 min)

### What I Did
- [ ] Scales:
- [ ] Arpeggios:
- [ ] Stretches:
- [ ] Other:

### Tempo/BPM
- Started at:
- Ended at:

---

## Technical Practice (20 min)

### Exercise 1:
- **What:**
- **Tempo:**
- **Key:**
- **Reps/Duration:**
- **Difficulty:** Easy / Medium / Hard / Struggle

**Notes:**


### Exercise 2:
- **What:**
- **Tempo:**
- **Reps/Duration:**
- **Difficulty:** Easy / Medium / Hard / Struggle

**Notes:**


---

## Song/Repertoire Work (20 min)

### Song:
- **Section worked on:**
- **Tempo:** Target: ___ | Achieved: ___
- **Problem spots:**


### Song:
- **Section worked on:**
- **Tempo:**
- **Problem spots:**


---

## Creative/Improvisation (10 min)

### What I explored
- **Key/Mode:**
- **Backing track:**
- **Ideas that emerged:**



---

## Ear Training

- [ ] Interval recognition
- [ ] Chord identification
- [ ] Melody transcription
- [ ] Rhythm dictation

**Notes:**


---

## Theory Study

### Topic:
**Key takeaway:**



---

## Wins 🎉

*What went well today?*

1.
2.
3.

---

## Struggles 😤

*What's still challenging?*

1.
2.

---

## Discoveries 💡

*New insights, techniques, or ideas*

-
-

---

## Tomorrow's Focus

*Based on today, what should I work on next?*

- [ ]
- [ ]

---

## Weekly Goal Progress

### This Week's Goal:


### Progress: ▓▓▓▓▓▓░░░░ 60%

---

## Notes



---

## Related
- [[Practice Log - <% tp.date.now("YYYY-MM-DD", -1) %>]]
- [[Theory/Music Theory Vocabulary]]

