...
export const SideBToolbar: React.FC = () => {
  ...
  const coreEmotion = (songIntent?.coreEmotion as string | undefined) || 'None';
  const subEmotion = (songIntent?.subEmotion as string | undefined) || 'None';
  const ruleToBreak = (songIntent?.ruleToBreak as string | undefined) || null;

  const handleGenerate = async () => {
    if (!songIntent) {
      console.warn('No song intent to process');
      return;
    }
    const result = await processIntent(songIntent);
    if (result) {
