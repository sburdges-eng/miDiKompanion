...
export const RuleBreaker: React.FC = () => {
  ...
  const currentRuleToBreak = (songIntent?.ruleToBreak as string | undefined) || null;
  const coreEmotion = (songIntent?.coreEmotion as string | undefined) || '';

  const handleSelectRule = (ruleId: string) => {
    const newIntent = {
      ...(songIntent || {}),
      ruleToBreak: currentRuleToBreak === ruleId ? null : ruleId,
    };
    updateSongIntent(newIntent);
  };

  const handleSuggest = async () => {
    if (coreEmotion) {
      await suggestRuleBreak(coreEmotion);
    }
  };
    ...
    <div className="p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          disabled={isLoading || !coreEmotion}
        >
        ...
        <div className="mt-3 p-2 bg-ableton-surface-light rounded flex items-center gap-2">
          ...
          <span className="text-sm text-ableton-text">
            ...
            <span className="text-ableton-accent">
              {RULE_BREAK_OPTIONS.find((r) => r.id === currentRuleToBreak)?.name}
            </span>
          </span>
        </div>
      )}
