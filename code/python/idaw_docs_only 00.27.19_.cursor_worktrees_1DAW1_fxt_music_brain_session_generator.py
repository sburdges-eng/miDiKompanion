...
class GeneratedSong:
    ...
    def export_to_midi(self, output_path: str) -> None:
        """
        ...
        try:
            ...
            raise ImportError("mido library required for MIDI export. Install with: pip install mido")

        # Create MIDI file
        mid = mido.MidiFile()
        mid.ticks_per_beat = 480  # Standard PPQ
        ...
        # Set tempo
        tempo_microseconds = int(60_000_000 / self.tempo_bpm)
        chord_track.append(mido.MetaMessage('set_tempo', tempo=tempo_microseconds, time=0))

        # Time signature
        beats_per_bar, beat_unit = self.time_signature
        chord_track.append(mido.MetaMessage('time_signature',
                                           numerator=beats_per_bar,
                                           denominator=beat_unit,
                                           time=0))

        # Key signature (mido format: sharps as positive, flats as negative)
        try:
            key_sharps = self._get_key_sharps(self.key)
            # mido expects key as string like "F" or number of sharps
            # For now, use a simple approach - just set to 0 (C major/A minor)
            # More complex key signatures can be added later
            chord_track.append(mido.MetaMessage('key_signature', key=0, time=0))
        except Exception:
            # Skip key signature if there's an error
            pass
        ...
        for section in self.sections:
            bars = section.bars
            chords_per_bar = len(section.progression)

            if chords_per_bar == 0:
                continue

            # Calculate ticks per chord
            ticks_per_chord = ticks_per_bar // chords_per_bar

            for bar in range(bars):
                for chord_idx, chord_name in enumerate(section.chords):
                    # Parse chord to get MIDI notes
                    notes = self._chord_to_midi_notes(chord_name, self.key, self.mode)

                    # Calculate velocity based on section energy
                    velocity = int(64 + (section.energy * 64))  # 64-127 range

                    # Note on for all notes in chord
                    for note in notes:
                        chord_track.append(mido.Message('note_on',
                                                       note=note,
                                                       velocity=velocity,
                                                       time=0 if current_tick == 0 else 0))

                    # Note off after chord duration
                    note_off_time = ticks_per_chord
                    for i, note in enumerate(notes):
                        chord_track.append(mido.Message('note_off',
                                                        note=note,
                                                        velocity=0,
                                                        time=note_off_time if i == 0 else 0))
    ...
    def _get_key_sharps(self, key: str) -> int:
        """Get MIDI key signature sharps (-7 to +7, negative = flats)."""
        key_map = {
            "C": 0, "G": 1, "D": 2, "A": 3, "E": 4, "B": 5, "F#": 6, "C#": 7,
            "F": -1, "Bb": -2, "Eb": -3, "Ab": -4, "Db": -5, "Gb": -6, "Cb": -7
        }
        # Handle keys with flats/sharps
        if key.endswith('b'):
            base = key[:-1]
            return key_map.get(key, key_map.get(base, 0) - 1)
        elif key.endswith('#'):
            base = key[:-1]
            return key_map.get(key, key_map.get(base, 0) + 1)
        return key_map.get(key, 0)

    def _chord_to_midi_notes(self, chord_name: str, key: str, mode: str) -> List[int]:
        """
        Convert chord name to MIDI note numbers.

        Args:
            chord_name: Chord name (e.g., "Fm", "C", "Am", "Bb")
            key: Key of the song
        ...
        """
        # Parse chord root - handle flats and sharps
        root = ""
        ...
        if 'b' in root:
            try:
                root_idx = note_names_flat.index(root)
            except ValueError:
        ...
        root_note = 60 + root_idx  # C4 = 60

        # Determine chord quality
        chord_lower = chord_name.lower()
        is_minor = 'm' in chord_lower and 'maj' not in chord_lower
        ...
        notes.append(root_note + 12)  # Add root an octave up
