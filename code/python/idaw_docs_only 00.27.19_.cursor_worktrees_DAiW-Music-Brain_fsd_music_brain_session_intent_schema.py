...
# Chromatic note constants (matching intent_processor)
CHROMATIC_NOTES = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
CHROMATIC_NOTES_FLAT = ['C', 'Db', 'D', 'Eb', 'E', 'F', 'Gb', 'G', 'Ab', 'A', 'Bb', 'B']

# Valid mode names
VALID_MODES = ["major", "minor", "Ionian", "Dorian", "Phrygian", "Lydian", "Mixolydian", "Aeolian", "Locrian"]


def get_valid_keys() -> List[str]:
    """
    Get list of valid musical key names.
    
    Returns:
        List of valid key names in sharp notation
    """
    return CHROMATIC_NOTES.copy()


def validate_musical_key(key: str) -> bool:
    """
    Validate musical key format.
    
    Supports formats like:
    - "C", "C#", "Db" (just the note)
    - "C major", "E minor" (note with quality)
    - "C# minor", "F# major" (sharp/flat with quality)
    ...
    # Split by space to separate note from quality
    parts = key.split()
    note_part = parts[0]
    
    # Check if note is valid (sharp or flat notation)
    if note_part in CHROMATIC_NOTES or note_part in CHROMATIC_NOTES_FLAT:
        # If there's a quality part, check it's valid
        if len(parts) > 1:
            quality = parts[1].lower()
            return quality in ["major", "minor"]
        # Just a note is also valid
        return True
...
def validate_mode(mode: str) -> bool:
    ...
    Accepts:
    - Simple modes: "major", "minor"
    - Modal modes: "Ionian", "Dorian", "Phrygian", "Lydian", "Mixolydian", "Aeolian", "Locrian"
    - Extended formats: "major with Lydian moments", "minor with Dorian color", etc.
    ...
    # Check if it's a simple valid mode
    if mode.lower() in [m.lower() for m in VALID_MODES]:
        return True
    
    # Check if it contains a valid mode (for extended formats like "major with Lydian moments")
    mode_lower = mode.lower()
    for valid_mode in VALID_MODES:
        if valid_mode.lower() in mode_lower:
            return True
...
def suggest_phase2_from_phase1(song_intent: SongIntent) -> Dict:
    """
    Suggest Phase 2 (Technical Constraints) values based on Phase 1 emotional intent.
    
    Uses AFFECT_MODE_MAP and suggest_rule_break() to generate intelligent suggestions.
    
    Args:
        song_intent: Phase 1 SongIntent data
    
    Returns:
        Dict with suggested values:
        - suggested_mode: List of suggested modes
        - suggested_tempo_range: Tuple of (min, max) BPM
        - suggested_rules: List of suggested rule-breaking options
    """
    result = {
        "suggested_mode": [],
        "suggested_tempo_range": (80, 120),  # Default
        "suggested_rules": [],
    }
    
    # Get affect mapping based on mood_primary
    if song_intent.mood_primary:
        mood_lower = song_intent.mood_primary.lower()
        
        # Try to match mood to AFFECT_MODE_MAP
        # Handle compound emotions (e.g., "Triumphant Hope" -> "hope")
        affect_mapping = None
        for affect_key in AFFECT_MODE_MAP.keys():
            if affect_key in mood_lower or mood_lower in affect_key:
                affect_mapping = get_affect_mapping(affect_key)
                if affect_mapping:
                    break
        
        # If no direct match, try first word
        if not affect_mapping:
            first_word = mood_lower.split()[0] if mood_lower.split() else mood_lower
            affect_mapping = get_affect_mapping(first_word)
        
        if affect_mapping:
            # Extract suggested modes
            modes = affect_mapping.get("modes", [])
            if modes:
                result["suggested_mode"] = modes
            
            # Extract suggested tempo range
            tempo_range = affect_mapping.get("tempo_range", (80, 120))
            if isinstance(tempo_range, (list, tuple)) and len(tempo_range) == 2:
                result["suggested_tempo_range"] = tuple(tempo_range)
        
        # Get rule-breaking suggestions
        rule_suggestions = suggest_rule_break(song_intent.mood_primary)
        if rule_suggestions:
            result["suggested_rules"] = [sug["rule"] for sug in rule_suggestions[:3]]  # Top 3
...
def collect_phase2_interactive(song_intent: Optional[SongIntent] = None) -> TechnicalConstraints:
    ...
    """
    options = load_schema_options()
    suggestions = suggest_phase2_from_phase1(song_intent) if song_intent else {}
    ...
    # Tempo Range
    print("\n2. TEMPO RANGE (BPM)")
    suggested_tempo = suggestions.get("suggested_tempo_range", (80, 120))
    print(f"   Suggested range: {suggested_tempo[0]}-{suggested_tempo[1]} BPM (based on Phase 1)")
    print("   Enter minimum and maximum BPM (40-240)")
    while True:
        try:
            tempo_input = input("   > ").strip()
            if not tempo_input:
                technical_tempo_range = suggested_tempo
                break
            
            # Parse "min-max" or "min, max" or just one number
            if "-" in tempo_input:
                ...
                single = int(tempo_input)
                if 40 <= single <= 240:
                    technical_tempo_range = (single, single)
                    break
                else:
                    print("   Please enter a value between 40 and 240")
                    continue
            
            if len(parts) == 2:
                min_bpm = int(parts[0].strip())
                max_bpm = int(parts[1].strip())
                if 40 <= min_bpm <= 240 and 40 <= max_bpm <= 240:
                    if min_bpm <= max_bpm:
                        technical_tempo_range = (min_bpm, max_bpm)
                        break
                    else:
                        print("   Minimum must be <= maximum")
                else:
                    print("   Please enter values between 40 and 240")
            else:
                print("   Please enter format: min-max or min,max (e.g., 80-120)")
        except ValueError:
    ...
    print("   You can enter just the note (e.g., 'C') or with quality (e.g., 'C major', 'E minor')")
    while True:
        key_input = input("   > ").strip()
        if not key_input:
            technical_key = "F"
            break
        if validate_musical_key(key_input):
            technical_key = key_input
            break
        else:
            print("   Invalid key format. Please enter a valid key (e.g., 'C', 'C#', 'E minor')")
    
    # Mode
    print("\n4. MODE")
    valid_modes = get_valid_modes()
    suggested_modes = suggestions.get("suggested_mode", [])
    if suggested_modes:
        print(f"   Suggested modes (from Phase 1): {', '.join(suggested_modes)}")
    print("   Valid modes:", ", ".join(valid_modes))
    print("   You can enter simple (major/minor) or modal (Dorian, Lydian, etc.)")
    print("   Extended formats allowed (e.g., 'major with Lydian moments')")
    while True:
        mode_input = input("   > ").strip()
        if not mode_input:
            technical_mode = suggested_modes[0] if suggested_modes else "major"
            break
        if validate_mode(mode_input):
            technical_mode = mode_input
            break
        else:
            print("   Invalid mode format. Please enter a valid mode")
    ...
    suggested_rules = suggestions.get("suggested_rules", [])
    if suggested_rules:
        print("   Suggested rules (from Phase 1 emotion):")
        for i, rule in enumerate(suggested_rules, 1):
            rule_info = get_rule_breaking_info(rule)
            if rule_info:
                print(f"   {i}. {rule}")
                print(f"      Effect: {rule_info.get('effect', '')}")
    print("   Enter a rule to break intentionally, or press Enter to skip")
    print("   Use 'list' to see all available rules")
    
    all_rules_dict = list_all_rules()
    all_rules_flat = []
    for category, rule_list in all_rules_dict.items():
        all_rules_flat.extend(rule_list)
        ...
        if rule_input.lower() == "list":
            ...
            for category, rule_list in all_rules_dict.items():
                ...
                for rule in rule_list:
                    print(f"     - {rule}")
            print("\n   Enter a rule to break, or press Enter to skip:")
            continue
        
        # Validate against all rule enums
        if rule_input in all_rules_flat:
            technical_rule_to_break = rule_input
            break
        else:
            print(f"   '{rule_input}' is not a valid rule.")
            print("   Use 'list' to see all available rules, or press Enter to skip")
    
    # Rule Breaking Justification
    rule_breaking_justification = ""
    if technical_rule_to_break:
        print("\n7. RULE BREAKING JUSTIFICATION (Required)")
        print("   WHY are you breaking this rule? What emotional effect does it serve?")
        print("   This justification connects the technical choice to the emotional intent.")
        while True:
            ...
            if justification_input:
                ...
                print("   Justification is required when a rule is specified.")
                print("   Explain the emotional/artistic reason for breaking this rule.")
...
def validate_phase2(technical_constraints: TechnicalConstraints) -> List[str]:
    ...
    if not isinstance(tempo_range, (tuple, list)) or len(tempo_range) != 2:
        issues.append("Phase 2: technical_tempo_range must be a tuple/list of two integers")
    else:
        try:
            min_bpm, max_bpm = int(tempo_range[0]), int(tempo_range[1])
            if min_bpm < 40 or min_bpm > 240:
                issues.append("Phase 2: technical_tempo_range minimum must be between 40 and 240 BPM")
            if max_bpm < 40 or max_bpm > 240:
                issues.append("Phase 2: technical_tempo_range maximum must be between 40 and 240 BPM")
            if min_bpm > max_bpm:
                issues.append("Phase 2: technical_tempo_range minimum must be <= maximum")
        except (ValueError, TypeError):
            issues.append("Phase 2: technical_tempo_range must contain two integers")
    ...
    elif not validate_musical_key(technical_constraints.technical_key):
        issues.append(f"Phase 2: technical_key '{technical_constraints.technical_key}' is not a valid key format")
    ...
    elif not validate_mode(technical_constraints.technical_mode):
        issues.append(f"Phase 2: technical_mode '{technical_constraints.technical_mode}' is not a valid mode format")
    ...
    if not technical_constraints.technical_groove_feel or technical_constraints.technical_groove_feel.startswith("["):
        ...
        # Warn but don't fail - allow custom groove feels
        pass
