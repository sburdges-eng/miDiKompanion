...
class TestCmdGenerate:
    ...
    def test_generate_basic_progression(self, capsys):
        """Test generating a basic progression from parameters."""
        args = Namespace(
            intent_file=None,
            key="F",
            mode="major",
        ...
        assert result == 0
        captured = capsys.readouterr()
        assert "Generated Harmony" in captured.out
        assert "F" in captured.out

    def test_generate_with_intent_file(self, valid_intent_file, capsys):
        """Test generating harmony from intent file."""
        args = Namespace(
            intent_file=valid_intent_file,
            key=None,
            mode=None,
            pattern=None,
            output=None,
        ...
        assert result == 0
        captured = capsys.readouterr()
        assert "Generated Harmony" in captured.out or "harmony" in captured.out.lower()

    def test_generate_with_output(self, tmp_path, capsys):
        """Test generating harmony with MIDI output."""
        ...
        assert result == 0
        captured = capsys.readouterr()
        assert "MIDI saved" in captured.out or output_file.exists()

    def test_generate_intent_file_not_found(self, capsys):
        """Test error handling for nonexistent intent file."""
        args = Namespace(
            intent_file="/nonexistent/file.json",
            key=None,
            mode=None,
            pattern=None,
            output=None,
    ...
    def test_generate_with_different_mode(self, capsys):
        """Test generating with different modes."""
    ...
    def test_generate_with_custom_tempo(self, tmp_path, capsys):
        """Test generating with custom tempo."""
        ...
        assert result == 0
        captured = capsys.readouterr()
        assert "140" in captured.out or "BPM" in captured.out
...
class TestCmdDiagnose:
    ...
    def test_diagnose_with_issues(self, capsys):
        args = Namespace(progression="C-C#-D-D#")
        result = cmd_diagnose(args)
        assert result == 0


def test_cmd_analyze_audio(monkeypatch, tmp_path):
    fake_analyzer = MagicMock()
    fake_analyzer.analyze_file.return_value = SimpleNamespace(
        bpm=120.0,
        key="C",
        mode="major",
        spectral=SimpleNamespace(
            ...
            harmonic_content={"low": 0.5, "high": 0.5},
        ),
        chords=["C", "G"],
    )

    monkeypatch.setattr("music_brain.cli.get_audio_module", lambda: lambda: fake_analyzer)

    audio_file = tmp_path / "demo.wav"
    audio_file.write_bytes(b"fake")

    args = Namespace(audio_file=str(audio_file))
    result = cmd_analyze_audio(args)

    assert result == 0
    fake_analyzer.analyze_file.assert_called_once()


def test_cmd_voice_tune(monkeypatch, tmp_path):
    class FakeAutoTuneSettings:
        def __init__(self):
            self.strength = 1.0
    ...
    class FakeProcessor:
        ...
        def process_file(self, input_path, output_path=None, key=None, mode="major"):
            return output_path or str(tmp_path / "tuned.wav")

    class FakeModSettings:
        pass
    ...
    class FakeSynth:
        ...
        def synthesize_guide(self, **kwargs):
            return kwargs.get("output_path", "guide.wav")

        def speak_text(self, **kwargs):
            return kwargs.get("output_path", "spoken.wav")

    def fake_get_voice_modules():
        return (
            FakeProcessor,
            FakeAutoTuneSettings,
            lambda preset: FakeAutoTuneSettings(),
            FakeModulator,
            FakeModSettings,
            lambda preset: FakeModSettings(),
            FakeSynth,
            FakeSynthConfig,
            lambda profile: FakeSynthConfig(),
        )

    monkeypatch.setattr("music_brain.cli.get_voice_modules", fake_get_voice_modules)

    input_file = tmp_path / "vox.wav"
    ...
    speak_args = Namespace(
        voice_cmd="speak",
        text="DAiW introduces Neon Ghost",
        output=None,
        profile="guide_confident",
        tempo=80,
    )
    result = cmd_voice(speak_args)
    assert result == 0


def test_cmd_chatbot(monkeypatch):
    created_agents = []
    ...
    monkeypatch.setattr("music_brain.cli.ChatAgent", FakeAgent)
    monkeypatch.setattr("builtins.input", lambda prompt="": next(inputs))

    args = Namespace(model="fake-model.gguf", persona="test persona")
    result = cmd_chatbot(args)

    assert result == 0
    assert created_agents
    assert created_agents[0].calls == ["hello there"]
