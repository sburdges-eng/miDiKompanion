...
# Valid keys for validation (both sharp and flat notation)
VALID_KEYS = ['C', 'C#', 'Db', 'D', 'D#', 'Eb', 'E', 'F', 'F#', 'Gb', 'G', 'G#', 'Ab', 'A', 'A#', 'Bb', 'B']

# Valid modes for validation
VALID_MODES = [
    "major", "minor",  # Basic modes
    "ionian", "dorian", "phrygian", "lydian", "mixolydian", "aeolian", "locrian",  # Church modes
]


def validate_phase2(technical_constraints: TechnicalConstraints) -> List[str]:
    """
    Validate Phase 2 fields (Technical Constraints).
    ...
    # Validate technical_genre (optional but should be from options if provided)
    if technical_constraints.technical_genre:
        if technical_constraints.technical_genre.startswith("["):
            issues.append("Phase 2: technical_genre placeholder not filled in")
        # Allow custom genres - just warn in CLI, not validation failure
    
    # Validate technical_tempo_range
    tempo_range = technical_constraints.technical_tempo_range
    if tempo_range:
        if not isinstance(tempo_range, (tuple, list)) or len(tempo_range) != 2:
            issues.append("Phase 2: technical_tempo_range must be a tuple/list of two integers (min, max)")
        else:
            min_tempo, max_tempo = tempo_range
            if not isinstance(min_tempo, (int, float)) or not isinstance(max_tempo, (int, float)):
                issues.append("Phase 2: technical_tempo_range values must be numbers")
            elif min_tempo >= max_tempo:
                issues.append("Phase 2: technical_tempo_range min must be less than max")
            elif min_tempo < 40 or max_tempo > 240:
                issues.append("Phase 2: technical_tempo_range should be between 40-240 BPM")
    
    # Validate technical_key
    if technical_constraints.technical_key:
        key = technical_constraints.technical_key
        if key.startswith("["):
            issues.append("Phase 2: technical_key placeholder not filled in")
        else:
            # Extract base key (remove "major", "minor", "m" suffix if present)
            base_key = key.split()[0].replace("m", "").replace("M", "")
            if len(base_key) > 2:
                base_key = base_key[:2] if base_key[1] in '#b' else base_key[0]
            
            # Normalize for comparison
            normalized_key = base_key.capitalize() if len(base_key) == 1 else base_key[0].upper() + base_key[1]
            
            if normalized_key not in VALID_KEYS:
                issues.append(f"Phase 2: technical_key '{key}' is not a valid key. Use: C, C#, Db, D, etc.")
    
    # Validate technical_mode
    if technical_constraints.technical_mode:
        mode = technical_constraints.technical_mode.lower()
        if technical_constraints.technical_mode.startswith("["):
            issues.append("Phase 2: technical_mode placeholder not filled in")
        else:
            # Extract first word (mode name) - allow descriptions like "major with Dorian color"
            mode_name = mode.split()[0].lower()
            if mode_name not in VALID_MODES:
                issues.append(f"Phase 2: technical_mode '{technical_constraints.technical_mode}' base mode not recognized. Use: major, minor, or modal names (Ionian, Dorian, etc.)")
    
    # Validate technical_groove_feel
    if technical_constraints.technical_groove_feel:
        if technical_constraints.technical_groove_feel.startswith("["):
            issues.append("Phase 2: technical_groove_feel placeholder not filled in")
        elif options["groove_feel"] and technical_constraints.technical_groove_feel not in options["groove_feel"]:
            # Allow custom groove feels - just warn
            pass
    
    # Validate technical_rule_to_break
    if technical_constraints.technical_rule_to_break:
        if technical_constraints.technical_rule_to_break.startswith("["):
            issues.append("Phase 2: technical_rule_to_break placeholder not filled in")
        else:
            # Check if rule exists in any category
            ...
            if technical_constraints.technical_rule_to_break not in all_rule_values:
                issues.append(f"Phase 2: technical_rule_to_break '{technical_constraints.technical_rule_to_break}' is not a valid rule. Use 'daiw intent list' to see options.")
    
    # Validate rule_breaking_justification (required if rule_to_break is set)
    if technical_constraints.technical_rule_to_break and not technical_constraints.technical_rule_to_break.startswith("["):
        if not technical_constraints.rule_breaking_justification or technical_constraints.rule_breaking_justification.startswith("["):
            issues.append("Phase 2: rule_breaking_justification is required when technical_rule_to_break is set - WHY break this rule?")
    
    return issues


# Mappings for Phase 1 → Phase 2 suggestions
IMAGERY_TO_GENRE = {
    "Sharp Edges": ["Industrial Pop", "Post-Punk", "Art Rock"],
    "Muffled": ["Lo-Fi Bedroom", "Dream Pop", "Shoegaze"],
    "Open/Vast": ["Cinematic Neo-Soul", "Chamber Pop", "Dream Pop"],
    "Claustrophobic": ["Industrial Pop", "Lo-Fi Bedroom", "Electronic"],
    "Hazy/Dreamy": ["Dream Pop", "Shoegaze", "Lo-Fi Bedroom"],
    "Crystalline": ["Chamber Pop", "Art Rock", "Electronic"],
    "Muddy/Thick": ["Lo-Fi Bedroom", "Shoegaze", "Alternative"],
    "Sparse/Empty": ["Confessional Acoustic", "Indie Folk", "Chamber Pop"],
    "Chaotic": ["Industrial Pop", "Post-Punk", "Electronic"],
    "Flowing/Liquid": ["Cinematic Neo-Soul", "R&B", "Dream Pop"],
    "Fractured": ["Post-Punk", "Art Rock", "Electronic"],
    "Warm/Enveloping": ["Cinematic Neo-Soul", "Indie Folk", "R&B"],
    "Cold/Distant": ["Synthwave", "Electronic", "Post-Punk"],
    "Blinding Light": ["Synthwave", "Electronic", "Euphoria"],
    "Deep Shadow": ["Industrial Pop", "Shoegaze", "Lo-Fi Bedroom"],
}

NARRATIVE_TO_GROOVE = {
    "Climb-to-Climax": ["Straight/Driving", "Push-Pull"],
    "Slow Reveal": ["Organic/Breathing", "Laid Back"],
    "Repetitive Despair": ["Mechanical", "Push-Pull"],
    "Static Reflection": ["Rubato/Free", "Laid Back"],
    "Sudden Shift": ["Syncopated", "Push-Pull"],
    "Descent": ["Organic/Breathing", "Laid Back"],
    "Rise and Fall": ["Push-Pull", "Organic/Breathing"],
    "Spiral": ["Syncopated", "Mechanical"],
}

VULNERABILITY_TO_RULES = {
    "Low": ["ARRANGEMENT_UnbalancedDynamics", "RHYTHM_MetricModulation"],
    "Medium": ["HARMONY_ModalInterchange", "RHYTHM_ConstantDisplacement"],
    "High": ["PRODUCTION_PitchImperfection", "ARRANGEMENT_BuriedVocals", "PRODUCTION_RoomNoise"],
}


def suggest_technical_constraints(song_intent: SongIntent, song_root: Optional[SongRoot] = None) -> Dict:
    """
    Generate Phase 2 technical constraint suggestions based on Phase 1 data.
    
    Uses emotional intent to suggest appropriate technical parameters.
    
    Args:
        song_intent: Phase 1 emotional intent data
        song_root: Optional Phase 0 data for additional context
    
    Returns:
        Dict with suggested values for all Phase 2 fields
    """
    suggestions = {
        "genre": [],
        "tempo_range": None,
        ...
        "groove_feel": [],
        "rule_to_break": [],
        "rule_justification_hints": [],
    }
    
    # Get affect mapping for mood_primary (tempo and mode suggestions)
    affect = get_affect_mapping(song_intent.mood_primary)
    if affect:
        suggestions["mode"] = affect.get("modes", [])
        suggestions["tempo_range"] = affect.get("tempo_range", (80, 120))
    
    # Get rule-breaking suggestions from emotion
    rule_suggestions = suggest_rule_break(song_intent.mood_primary)
    for rule_sugg in rule_suggestions[:5]:  # Limit to top 5
        suggestions["rule_to_break"].append(rule_sugg["rule"])
        suggestions["rule_justification_hints"].append({
            "rule": rule_sugg["rule"],
            "use_when": rule_sugg.get("use_when", ""),
            "effect": rule_sugg.get("effect", ""),
        })
    
    # Map imagery_texture to genre suggestions
    if song_intent.imagery_texture in IMAGERY_TO_GENRE:
        suggestions["genre"] = IMAGERY_TO_GENRE[song_intent.imagery_texture]
    
    # Map narrative_arc to groove feel
    if song_intent.narrative_arc in NARRATIVE_TO_GROOVE:
        suggestions["groove_feel"] = NARRATIVE_TO_GROOVE[song_intent.narrative_arc]
    
    # Map vulnerability_scale to additional rule suggestions
    if song_intent.vulnerability_scale in VULNERABILITY_TO_RULES:
        vuln_rules = VULNERABILITY_TO_RULES[song_intent.vulnerability_scale]
        for rule in vuln_rules:
            if rule not in suggestions["rule_to_break"]:
                suggestions["rule_to_break"].append(rule)
    
    # Suggest keys based on mode (common associations)
    mode_key_map = {
        "Aeolian": ["Am", "Em", "Dm"],
        "Phrygian": ["E", "A", "B"],
        "Dorian": ["Dm", "Am", "Em"],
    ...
    for mode in suggestions["mode"]:
        if mode in mode_key_map:
            for key in mode_key_map[mode]:
                if key not in suggestions["key"]:
                    suggestions["key"].append(key)
    
    # Default suggestions if nothing matched
    if not suggestions["genre"]:
        suggestions["genre"] = ["Alternative", "Indie Folk", "Electronic"]
    if not suggestions["tempo_range"]:
        suggestions["tempo_range"] = (80, 120)
    if not suggestions["mode"]:
        suggestions["mode"] = ["minor", "Aeolian"]
    if not suggestions["groove_feel"]:
        suggestions["groove_feel"] = ["Organic/Breathing", "Laid Back"]
    if not suggestions["key"]:
        suggestions["key"] = ["F", "Am", "C"]
...
def collect_phase2_interactive(
    ...
    Shows suggestions based on Phase 1 data if available.
    ...
    # Get suggestions if Phase 1 data available
    if song_intent:
        suggestions = suggest_technical_constraints(song_intent, song_root)
    ...
    if song_intent and song_intent.mood_primary:
        print(f"(Suggestions based on Phase 1: {song_intent.mood_primary})\n")
    
    # 1. Genre
    print("1. GENRE")
    print("   What genre/style?")
    if suggestions.get("genre"):
        print(f"   Suggested: {', '.join(suggestions['genre'][:3])}")
    if options["genre"]:
        print(f"   Options: {', '.join(options['genre'][:8])}...")
    technical_genre = input("   > ").strip()
    if not technical_genre:
        technical_genre = suggestions.get("genre", ["Alternative"])[0] if suggestions.get("genre") else ""
    
    # 2. Tempo Range
    print("\n2. TEMPO RANGE (BPM)")
    print("   Enter min and max tempo (e.g., '80 120' or '80-120')")
    if suggestions.get("tempo_range"):
        print(f"   Suggested: {suggestions['tempo_range'][0]}-{suggestions['tempo_range'][1]} BPM")
    while True:
        tempo_input = input("   > ").strip()
        if not tempo_input:
            if suggestions.get("tempo_range"):
                technical_tempo_range = tuple(suggestions["tempo_range"])
            else:
                technical_tempo_range = (80, 120)
            break
        
        # Parse input (accept "80 120", "80-120", "80, 120")
        parts = tempo_input.replace("-", " ").replace(",", " ").split()
        if len(parts) == 2:
            try:
                min_tempo = int(parts[0])
                max_tempo = int(parts[1])
                if min_tempo < max_tempo and 40 <= min_tempo <= 240 and 40 <= max_tempo <= 240:
                    technical_tempo_range = (min_tempo, max_tempo)
                    break
                else:
                    print("   Invalid range. Min must be < max, both between 40-240 BPM")
            except ValueError:
                print("   Please enter two numbers (e.g., '80 120')")
        else:
            print("   Please enter two numbers (e.g., '80 120' or '80-120')")
    
    # 3. Key
    print("\n3. MUSICAL KEY")
    print("   Enter key (C, C#, Db, D, etc.)")
    if suggestions.get("key"):
        print(f"   Suggested: {', '.join(suggestions['key'][:3])}")
    while True:
        key_input = input("   > ").strip()
        if not key_input:
            technical_key = suggestions.get("key", ["F"])[0] if suggestions.get("key") else "F"
            break
        
        # Validate key
        base_key = key_input.split()[0]
        if len(base_key) > 2:
            base_key = base_key[:2] if len(base_key) > 1 and base_key[1] in '#b' else base_key[0]
        normalized = base_key.capitalize() if len(base_key) == 1 else base_key[0].upper() + base_key[1]
        
        if normalized in VALID_KEYS:
            technical_key = key_input
            break
        else:
            print(f"   Invalid key. Use: C, C#, Db, D, D#, Eb, E, F, F#, Gb, G, G#, Ab, A, A#, Bb, B")
    
    # 4. Mode
    print("\n4. MODE")
    print("   Enter mode (major, minor, Ionian, Dorian, Phrygian, Lydian, Mixolydian, Aeolian, Locrian)")
    if suggestions.get("mode"):
        print(f"   Suggested: {', '.join(suggestions['mode'][:2])}")
    while True:
        mode_input = input("   > ").strip()
        if not mode_input:
            technical_mode = suggestions.get("mode", ["major"])[0] if suggestions.get("mode") else "major"
            break
        
        # Validate mode (just first word)
        mode_name = mode_input.split()[0].lower()
        if mode_name in VALID_MODES:
            technical_mode = mode_input
            break
        else:
            print(f"   Invalid mode. Use: major, minor, or modal names")
    
    # 5. Groove Feel
    print("\n5. GROOVE FEEL")
    print("   How should the rhythm feel?")
    if suggestions.get("groove_feel"):
        print(f"   Suggested: {', '.join(suggestions['groove_feel'][:2])}")
    if options["groove_feel"]:
        print(f"   Options: {', '.join(options['groove_feel'])}")
    technical_groove_feel = input("   > ").strip()
    if not technical_groove_feel:
        technical_groove_feel = suggestions.get("groove_feel", ["Organic/Breathing"])[0] if suggestions.get("groove_feel") else ""
    
    # 6. Rule to Break
    print("\n6. RULE TO BREAK (optional)")
    print("   Which rule do you want to intentionally break?")
    if suggestions.get("rule_to_break"):
        print(f"   Suggested for '{song_intent.mood_primary if song_intent else 'your emotion'}':")
        for i, rule in enumerate(suggestions["rule_to_break"][:5], 1):
            hint = next((h for h in suggestions.get("rule_justification_hints", []) if h["rule"] == rule), None)
            effect = hint.get("effect", "") if hint else ""
            print(f"      {i}. {rule}" + (f" - {effect}" if effect else ""))
    print("   (Enter rule name, number from suggestions, 'list' to see all, or leave blank)")
    ...
    while True:
        ...
        if rule_input.lower() == "list":
            # Show all rules by category
            all_rules = list_all_rules()
            print("\n   All available rules by category:")
            for category, rules in all_rules.items():
                print(f"\n   {category}:")
                for rule in rules:
                    info = RULE_BREAKING_EFFECTS.get(rule, {})
                    desc = info.get("description", "")[:40]
                    print(f"      - {rule}" + (f": {desc}..." if desc else ""))
            print("\n   Enter rule name:")
            continue
        
        # Check if number from suggestions
        if rule_input.isdigit():
            idx = int(rule_input) - 1
            if suggestions.get("rule_to_break") and 0 <= idx < len(suggestions["rule_to_break"]):
                technical_rule_to_break = suggestions["rule_to_break"][idx]
                break
        
        # Check if valid rule name
        all_rules = list_all_rules()
        all_rule_values = []
        for category_rules in all_rules.values():
            all_rule_values.extend(category_rules)
        
        if rule_input in all_rule_values:
            technical_rule_to_break = rule_input
            break
        else:
            print(f"   Rule '{rule_input}' not found. Type 'list' to see all options.")
    
    # 7. Rule Breaking Justification (if rule selected)
    if technical_rule_to_break:
        print("\n7. RULE BREAKING JUSTIFICATION")
        print("   WHY are you breaking this rule? What emotional effect are you after?")
        info = RULE_BREAKING_EFFECTS.get(technical_rule_to_break, {})
        if info.get("use_when"):
            print(f"   Hint: This rule is often broken when: {info['use_when']}")
        if info.get("effect"):
        ...
        while True:
            ...
            if justification:
                ...
                print("   Justification is required when breaking a rule. Why this rule?")
