...
def get_intent_module():
    from music_brain.session.intent_schema import (
        ...
        collect_phase1_interactive, validate_phase1,
        collect_phase2_interactive, validate_phase2, suggest_technical_constraints
    )
...
def cmd_intent(args):
    ...
    (CompleteSongIntent, SongRoot, SongIntent, TechnicalConstraints,
     SystemDirective, suggest_rule_break, validate_intent, list_all_rules,
     IntentProcessor, process_intent, collect_phase1_interactive, validate_phase1,
     collect_phase2_interactive, validate_phase2, suggest_technical_constraints) = get_intent_module()
    ...
    if args.subcommand == 'new':
        ...
        if intent_path and intent_path.exists():
            try:
                ...
                if song_intent.mood_primary:
                    print(f"Phase 1 data found: {song_intent.mood_primary}")
                    print("Phase 2 suggestions will be based on your emotional intent.\n")
            except Exception as e:
                print(f"Warning: Could not load intent file: {e}")
                print("Starting fresh Phase 2 collection.\n")
        
        # Collect Phase 2 interactively
        technical_constraints = collect_phase2_interactive(song_intent=song_intent, song_root=song_root)
        ...
        # Save or update intent
        if intent_path and intent_path.exists() and intent:
            # Update existing intent
            ...
            all_issues = validate_intent(intent)
            if not all_issues:
                print(f"\n🎵 Intent is complete! Run:")
                print(f"   daiw intent process {intent_path}")
            else:
                print(f"\nRemaining issues:")
            ...
            print(f"\nNext steps:")
            print(f"  1. Fill Phase 0: Edit {output_path}")
            print(f"  2. Fill Phase 1: daiw intent phase1 -f {output_path}")
            print(f"  3. Process: daiw intent process {output_path}")
