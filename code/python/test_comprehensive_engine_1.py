...
@pytest.mark.parametrize("keyword, expected_affect", [
    ...
    ("god", "awe"),
    ("infinite", "awe"),
    ("wonder", "awe"),  # Fixed: 'divine' is not a keyword, 'wonder' is
    # Nostalgia keywords
    ("remember", "nostalgia"),
    ("childhood", "nostalgia"),
    ("memory", "nostalgia"),
    ...
    ("floating", "dissociation"),  # Fixed: 'empty' is in grief, not dissociation
    # Defiance keywords
    ("refuse", "defiance"),
    ("strong", "defiance"),
    ("won't", "defiance"),  # Fixed: 'fight' is in rage, not defiance
    # Tenderness keywords
    ("gentle", "tenderness"),
    ("care", "tenderness"),
    ("soft", "tenderness"),
    ...
    ("chaos", "confusion"),
    ("why", "confusion"),
    ("lost", "confusion"),  # Fixed: 'confused' is not a direct keyword
])
...
def test_generate_plan_complexity_from_motivation(session):
    """Higher motivation should result in more complexity."""
    session.process_core_input("test")
    ...
    # High motivation adds complexity (+0.1)
    assert high_mot_plan.complexity >= low_mot_plan.complexity
...
def test_harmony_plan_creation():
    """HarmonyPlan should be created with all required fields."""
    plan = HarmonyPlan(
        root_note="C",
        mode="minor",
        tempo_bpm=120,
...
def test_harmony_plan_progression_from_therapy():
    """Chord symbols should be generated by TherapySession."""
    session = TherapySession()
    session.process_core_input("I feel sad")
    session.set_scales(5, 0.5)
    plan = session.generate_plan()
    assert len(plan.chord_symbols) > 0
