...
async def interrogate(request: InterrogateRequest):
    ...
    try:
        from music_brain.interrogator import Interrogator
        import uuid

        # Initialize interrogator (singleton pattern)
        if not hasattr(interrogate, "_interrogator"):
            interrogate._interrogator = Interrogator()

        interrogator = interrogate._interrogator

        # Get or create session ID
        session_id = request.session_id or str(uuid.uuid4())

        # Process message
        result = interrogator.process_message(session_id, request.message)
