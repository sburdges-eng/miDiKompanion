...
#include "music_brain/emidi/Scheduler.hpp"
...
PYBIND11_MODULE(emidi_cpp, m) {
    ...
    py::class_<emidi::ScheduledEvent>(m, "ScheduledEvent")
        .def(py::init<>())
        .def_readwrite("start_tick", &emidi::ScheduledEvent::start_tick)
        .def_readwrite("event", &emidi::ScheduledEvent::event)
        .def_readwrite("channel", &emidi::ScheduledEvent::channel)
        .def_readwrite("event_id", &emidi::ScheduledEvent::event_id)
        .def_readwrite("metadata", &emidi::ScheduledEvent::metadata);

    py::class_<emidi::EventScheduler>(m, "EventScheduler")
        .def(py::init<>())
        .def("clear", &emidi::EventScheduler::clear)
        .def("size", &emidi::EventScheduler::size)
        .def("schedule", &emidi::EventScheduler::schedule, py::arg("event"))
        .def("schedule_many", &emidi::EventScheduler::schedule_many, py::arg("events"))
        .def("peek_next_tick", &emidi::EventScheduler::peek_next_tick)
        .def("pop_due_events", &emidi::EventScheduler::pop_due_events, py::arg("current_tick"), py::arg("lookahead_ticks"));
