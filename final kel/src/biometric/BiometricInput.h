...
public:
     ...
     */
    void setAdaptiveNormalization(bool enabled) { adaptiveNormalization_ = enabled; }
...
private:
    ...
    // Bridge instances for hardware integration (forward declared to avoid circular includes)
    class HealthKitBridge;
    class FitbitBridge;
    std::unique_ptr<HealthKitBridge> healthKitBridge_;
    std::unique_ptr<FitbitBridge> fitbitBridge_;
