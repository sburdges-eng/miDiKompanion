...
VocalExpression ExpressionEngine::applyEmotionExpression(
    ...
    const EmotionNode& emotion)
{
    ...
    expression.vibratoRate = baseExpression.vibratoRate + (emotion.arousal * 2.0f);
    expression.vibratoRate = std::clamp(expression.vibratoRate, 2.0f, 8.0f);

    // Intensity affects vibrato depth
    expression.vibratoDepth = baseExpression.vibratoDepth + (emotion.intensity * 0.2f);
    ...
    expression.articulation = baseExpression.articulation + ((1.0f - emotion.arousal) * 0.4f);
...
void ExpressionEngine::mapEmotionToExpression(
    const EmotionNode& emotion,
    VocalExpression& expression) const
{
    ...
    int numPoints) const
{
    ...
    for (int i = 0; i < numPoints; ++i) {
        ...
        VocalExpression posExpression = getExpressionAtPosition(expression, position);
...
std::vector<float> ExpressionEngine::generateDynamicsCurve(
    double duration,
    ...
    int numPoints) const
{
    ...
    for (int i = 0; i < numPoints; ++i) {
        float position = static_cast<float>(i) / static_cast<float>(numPoints - 1);
        float value = applyDynamicsCurve(position, crescendoAmount, diminuendoAmount);
        curve.push_back(value);
    }
...
std::vector<float> ExpressionEngine::generateVibratoCurve(
    double duration,
    float baseDepth,
    ...
    int numPoints) const
{
    ...
    for (int i = 0; i < numPoints; ++i) {
        ...
        float variation = std::sin(position * 2.0f * static_cast<float>(M_PI) * 3.0f) * variationAmount;
...
VocalExpression ExpressionEngine::getExpressionAtPosition(
    const VocalExpression& expression,
    float position) const
{
    ...
    float dynamicsMultiplier = applyDynamicsCurve(position, expression.crescendo, expression.diminuendo);
    ...
    float vibratoVariation = std::sin(position * 2.0f * static_cast<float>(M_PI) * 2.0f) * 0.1f;
    posExpression.vibratoDepth += vibratoVariation;
    posExpression.vibratoDepth = std::clamp(posExpression.vibratoDepth, 0.0f, 1.0f);

    return posExpression;
}

float ExpressionEngine::applyDynamicsCurve(
    float position,
    float crescendo,
