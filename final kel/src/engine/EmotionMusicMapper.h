...
/**
 * EmotionMusicMapper - Precise mathematical formulas for emotion-to-music translation.
 *
...
public:
    ...
    static std::string calculateDetailedMode(float valence, float arousal) {
        if (valence > 0.5f) {
            // Positive emotions
            if (arousal > 0.7f) return "lydian";      // Bright, excited
            if (arousal > 0.4f) return "ionian";      // Standard major
            ...
            // Strongly negative
            if (arousal > 0.7f) return "phrygian";    // Intense, dark
    ...
    static float calculateReward(const TherapeuticFactors& factors) {
        return 0.4f * factors.emotionalExpression +
               0.3f * factors.catharsis +
               0.2f * factors.narrative +
               0.1f * factors.flow;
    }
    ...
    static float calculateResonance(const BiometricState& state) {
        return 0.3f * state.deltaHRV +
               0.2f * state.deltaEDA +
               0.3f * state.valence +
               0.2f * state.complexity;
    }
    ...
    static MusicalParameters mapEmotion(
        ...
        if (dominance == 0.5f) {  // Default value, estimate instead
            dominance = estimateDominance(emotion.category, emotion.intensity);
        }
        params.velocity = calculateVelocity(dominance);

        params.mode = calculateMode(emotion.valence);
        params.detailedMode = calculateDetailedMode(emotion.valence, emotion.arousal);
