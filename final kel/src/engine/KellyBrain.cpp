...
// KellyBrain.h includes KellyTypes.h, so Wound, EmotionNode, etc. are KellyTypes versions
// Now we create aliases for the KellyTypes versions before Types.h redefines them
namespace kelly {
        ...
        for (const auto& rb : legacy.ruleBreaks) {
            ...
            switch (rb.type) {
                ...
                case RuleBreakType::DynamicContrast:
                    unifiedRb.type = static_cast<KellyTypesRuleBreakType>(6);  // DynamicContrast
                    break;
                ...
                case RuleBreakType::HarmonicAmbiguity:
                    unifiedRb.type = static_cast<KellyTypesRuleBreakType>(7);  // HarmonicAmbiguity
                    break;
                default:
...
KellyTypesIntentResult KellyBrain::fromJourney(const SideA& current, const SideB& desired) {
...
KellyTypesIntentResult KellyBrain::fromEmotion(const std::string& emotionName, float intensity) {
    ...
    if (emotionOpt) {
        ...
        wound.primaryEmotion.categoryEnum = emotionOpt->category;
        wound.primaryEmotion.category = categoryToString(emotionOpt->category);
...
GeneratedMidi KellyBrain::generateMidiFromWound(const KellyTypesWound& wound, int bars) {
    KellyTypesIntentResult result = fromWound(wound);
    return generateMidi(result, bars);
}

KellyTypesEmotionNode KellyBrain::resolveEmotionByName(const std::string& emotionName) {
    ...
    if (emotionOpt) {
        ...
        unified.categoryEnum = emotionOpt->category;
        unified.category = categoryToString(emotionOpt->category);
...
KellyTypesWound KellyBrain::descriptionToWound(const std::string& description, float intensity) {
    KellyTypesWound wound;
...
IntentPipeline& KellyBrain::pipeline() {
    return *pipeline_;
}

const IntentPipeline& KellyBrain::pipeline() const {
    return *pipeline_;
}

IntentPipeline& KellyBrain::getIntentPipeline() {
    return *pipeline_;
}
...
EmotionThesaurus& KellyBrain::thesaurus() {
