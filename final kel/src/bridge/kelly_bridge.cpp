...
PYBIND11_MODULE(kelly_bridge, m) {
    ...
    py::enum_<RuleBreakType>(m, "RuleBreakType")
        .value("Harmony", RuleBreakType::Harmony)
        .value("Rhythm", RuleBreakType::Rhythm)
        .value("Dynamics", RuleBreakType::Dynamics)
        .value("Melody", RuleBreakType::Melody)
        .value("Form", RuleBreakType::Form);
    ...
    py::class_<Wound>(m, "Wound")
        .def(py::init<>())
        .def(py::init<const std::string&, float, const std::string&>(),
             py::arg("description"), py::arg("intensity") = 0.7f,
             py::arg("source") = "internal")
        ...
        .def("__repr__", [](const Wound& w) {
            return "Wound(description='" + w.description +
                   "', intensity=" + std::to_string(w.intensity) +
                   ", source='" + w.source + "')";
        });
    ...
    py::class_<EmotionNode>(m, "EmotionNode")
        ...
        .def("__repr__", [](const EmotionNode& e) {
            return "EmotionNode(id=" + std::to_string(e.id) +
                   ", name='" + e.name +
    ...
    py::class_<RuleBreak>(m, "RuleBreak")
        ...
        .def_readonly("severity", &RuleBreak::severity)
        .def_readonly("description", &RuleBreak::description)
        .def_readonly("reason", &RuleBreak::reason)
    ...
    py::class_<IntentResult>(m, "IntentResult")
        .def_readonly("wound", &IntentResult::wound)
    ...
    py::class_<IntentPipeline>(m, "IntentPipeline")
        .def(py::init<>())
        .def("process", &IntentPipeline::process,
             py::arg("wound"),
        ...
        .def("process_journey", &IntentPipeline::processJourney,
             ...
             "Process Side A (current state) and Side B (desired state) to create a musical journey")
        .def("thesaurus", [](IntentPipeline& self) -> const EmotionThesaurus& {
    ...
    py::class_<EmotionThesaurus>(m, "EmotionThesaurus")
        .def("find_by_id", [](EmotionThesaurus& self, int id) -> py::object {
            ...
            py::arg("id"),
            "Find emotion by ID")
        .def("find_by_name", [](EmotionThesaurus& self, const std::string& name) -> py::object {
        ...
        .def("find_nearest", [](EmotionThesaurus& self, float v, float a, float i) {
        ...
        .def("find_nearest_vad", [](EmotionThesaurus& self, float v, float a, float d) {
        ...
        .def("find_related", [](EmotionThesaurus& self, int emotionId) {
                return self.findRelated(emotionId);
            },
            py::arg("emotion_id"),
            "Find emotions related to a given emotion")
        .def("find_by_category", [](EmotionThesaurus& self, EmotionCategory cat) {
    ...
    m.def("midi_note_to_name", [](int noteNumber) -> std::string {
        static const char* noteNames[] = {"C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"};
        int octave = (noteNumber / 12) - 1;
        int noteIndex = noteNumber % 12;
        return std::string(noteNames[noteIndex]) + std::to_string(octave);
    }, py::arg("note_number"), "Convert MIDI note number to note name (e.g., 60 -> 'C4')");

    m.def("note_name_to_midi", [](const std::string& name) -> int {
    ...
    m.def("category_to_string", [](EmotionCategory category) -> std::string {
        switch (category) {
