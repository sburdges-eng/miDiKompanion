...
// Create aliases before including EmotionThesaurus.h (which includes Types.h via IntentPipeline.h)
namespace kelly {
...
// Now include EmotionThesaurus.h - this brings in Types.h via IntentPipeline.h
// Note: We need Types.h types for EmotionThesaurus, so we include it after creating aliases
// categoryToString is in KellyTypes.h which is already included via MLBridge.h
...
namespace {
    KellyTypesEmotionNode convertToUnifiedEmotionNode(const EmotionNode& legacy) {
        ...
        unified.categoryEnum = legacy.category;
        unified.category = categoryToString(legacy.category);
...
KellyTypesEmotionNode EmotionEmbeddingMapper::embeddingToEmotion(const std::array<float, 64>& embedding) {
...
std::array<float, 64> EmotionEmbeddingMapper::emotionToEmbedding(const KellyTypesEmotionNode& node) {
...
KellyTypesIntentResult MLIntentPipeline::processAudio(const float* audioData, size_t numSamples) {
    ...
    KellyTypesEmotionNode emotion = mapper_->embeddingToEmotion(mlResult.emotionEmbedding);
...
KellyTypesIntentResult MLIntentPipeline::processHybrid(
    ...
    // Use audio emotion intensity (KellyTypes::IntentResult uses sourceWound.primaryEmotion)
    blended.sourceWound.primaryEmotion.intensity =
        (audioResult.sourceWound.primaryEmotion.intensity + textResult.sourceWound.primaryEmotion.intensity) / 2.0f;
...
std::vector<Chord> MLIntentPipeline::generateHarmonyFromPrediction(
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        chord.quality = "major";
        chord.root = "C";
        // KellyTypes::Chord doesn't have rootNote, startTick, durationTicks - adjust structure
        // For now, just set the pitches and symbol
...
void MLIntentPipeline::applyDynamics(
    ...
    for (size_t i = 0; i < notes.size(); ++i) {
        ...
        float velocityCurve = baseVelocityMod + std::sin(positionInPhrase * 3.14159f) * 0.3f;
        notes[i].velocity = std::clamp(
            static_cast<int>(notes[i].velocity * (0.7f + velocityCurve * 0.6f)),
            1, 127);
    }
}
...
GeneratedMidi MLIntentPipeline::generateFromAudio(
    ...
    result.tempoBpm = intent.tempoBpm;  // KellyTypes::IntentResult has tempoBpm (int)
    result.bars = bars;
    result.key = intent.key;
    result.mode = intent.mode;
    // KellyTypes::GeneratedMidi doesn't have bpm or lengthInBeats fields - they're in Types::GeneratedMidi
    // Store tempo in metadata if needed
...
void MLIntentPipeline::setModelEnabled(Kelly::ML::ModelType type, bool enabled) {
