...
public:
     ...
     */
    std::array<float, FEATURE_SIZE> extractFeatures(
        ...
        const int actualNumSamples = (numSamples < 0) ? buffer.getNumSamples() : numSamples;
        const int endSample = std::min(startSample + actualNumSamples, buffer.getNumSamples());
        ...
        float spectralCentroid = computeSpectralCentroid(channelData, startSample, actualLength);
        features[featureIdx++] = spectralCentroid;

        // 4. Spectral rolloff
        float spectralRolloff = computeSpectralRolloff(channelData, startSample, actualLength);
        features[featureIdx++] = spectralRolloff;

        // 5. Spectral flux
        float spectralFlux = computeSpectralFlux(channelData, startSample, actualLength);
        features[featureIdx++] = spectralFlux;

        // 6. MFCC-like features (simplified)
        extractMFCCFeatures(channelData, startSample, actualLength, features, featureIdx);
        featureIdx += 13;  // 13 MFCC coefficients

        // 7. Spectral features (FFT-based)
        extractSpectralFeatures(channelData, startSample, actualLength, features, featureIdx);
        featureIdx += 64;  // 64 spectral bins

        // 8. Temporal features
        extractTemporalFeatures(channelData, startSample, actualLength, features, featureIdx);
        featureIdx += 20;  // 20 temporal features

        // 9. Harmonic features
        extractHarmonicFeatures(channelData, startSample, actualLength, features, featureIdx);
...
private:
    float computeSpectralCentroid(const float* data, int start, int length) const {
    ...
    void extractMFCCFeatures(
        ...
        for (int i = 0; i < 13; ++i) {
            ...
            for (int j = 0; j < length; ++j) {
                ...
                sum += data[start + j] * std::cos(static_cast<float>(i) * freq * 2.0f * 3.14159f);
            }
    ...
    void extractTemporalFeatures(
        ...
        // Decay rate
        if (maxIdx < length - 1) {
            float decay = (maxVal - std::abs(data[start + length - 1])) / (length - maxIdx);
    ...
    void extractHarmonicFeatures(
        ...
        // Simplified harmonic features
        // In a full implementation, use autocorrelation or FFT-based pitch detection
        for (int i = 0; i < 20; ++i) {
            features[startIdx + i] = 0.0f;  // Placeholder
        }
    }
};
