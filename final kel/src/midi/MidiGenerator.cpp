...
GeneratedMidi MidiGenerator::generate(
    ...
    if (bars >= 8) {  // Only generate arrangement for longer pieces
        ...
        static thread_local ArrangementOutput storedArrangement;  // Thread-local storage for pointer
    ...
    if (layers.counterMelody && !result.melody.empty()) {
        result.counterMelody = generateCounterMelody(result.melody, intent, complexity);
    }
...
std::vector<Chord> MidiGenerator::generateChords(const IntentResult& intent, int bars) {
...
std::vector<MidiNote> MidiGenerator::generateMelody(
    ...
    if (!musicalParams.detailedMode.empty()) mode = musicalParams.detailedMode;
    if (!musicalParams.mode.empty()) key = musicalParams.mode;
    ...
    config.key = key;
    config.mode = mode;
    config.bars = static_cast<int>(chords.size() / static_cast<double>(BEATS_PER_BAR) + 0.5); // Approximate bars from chords
    ...
    for (auto& note : melody) {
        note.velocity = std::clamp(
            static_cast<int>(note.velocity * dynamics),
...
std::vector<MidiNote> MidiGenerator::generatePads(
    const std::vector<Chord>& chords,
    const IntentResult& intent,
    float complexity
) {
...
std::vector<MidiNote> MidiGenerator::generateStrings(
    const std::vector<Chord>& chords,
    const IntentResult& intent,
    float complexity
) {
...
std::vector<MidiNote> MidiGenerator::generateCounterMelody(
...
std::vector<MidiNote> MidiGenerator::generateRhythm(
    const IntentResult& intent,
    int bars,
    float complexity,
    int tempoBpm
...
std::vector<MidiNote> MidiGenerator::generateDrumGroove(
    const IntentResult& intent,
    int bars,
    float complexity,
    int tempoBpm
...
std::vector<MidiNote> MidiGenerator::generateTransitions(
    const IntentResult& intent,
    int bars,
    int tempoBpm
) {
...
std::vector<MidiNote> MidiGenerator::generateFills(
    const IntentResult& intent,
    int bars,
    float complexity
) {
    ...
    // Convert DrumHits to MidiNotes (for fills, we might want to keep as drum hits)
    // For now, we'll convert them
    for (const auto& hit : output.hits) {
...
ArrangementOutput MidiGenerator::generateArrangement(
    const IntentResult& intent,
    int bars
) {
...
void MidiGenerator::applyDynamics(
    GeneratedMidi& midi,
    const IntentResult& intent,
    float dynamics
) {
    ...
    if (!midi.melody.empty()) {
        ...
        config.totalTicks = beatsToTicks(midi.lengthInBeats, static_cast<int>(midi.bpm));
...
void MidiGenerator::applyTension(
    GeneratedMidi& midi,
    const IntentResult& intent,
    int bars
) {
    ...
    for (size_t i = 0; i < midi.chords.size() && i < output.tensionCurve.size(); ++i) {
        ...
        if (tension > INTENSITY_MODERATE && !midi.chords[i].pitches.empty()) {
            ...
            if (std::find(midi.chords[i].pitches.begin(), midi.chords[i].pitches.end(), tensionNote)
...
void MidiGenerator::applyRuleBreaks(
    GeneratedMidi& midi,
    const IntentResult& intent
) {
    for (const auto& ruleBreak : intent.ruleBreaks) {
        // RuleBreak from Types.h has: type, severity, description, reason
        ...
        switch (ruleBreak.type) {
                    ...
                    for (auto& chord : midi.chords) {
                        if (!chord.pitches.empty() && severity > RULE_BREAK_MODERATE) {
                            ...
                            if (dist(rng_) < severity) {
                                ...
                                if (std::find(chord.pitches.begin(), chord.pitches.end(), clusterNote)
                                    == chord.pitches.end()) {
            ...
            case RuleBreakType::Rhythm: {
                ...
                if (severity > RULE_BREAK_LOW) {
                    ...
                    for (auto& note : midi.bass) {
                        std::uniform_real_distribution<float> dist(-syncAmount * BASS_HUMANIZE_MULTIPLIER, syncAmount * BASS_HUMANIZE_MULTIPLIER);
                        note.startBeat += dist(rng_);
                    }
                }
                break;
            ...
            case RuleBreakType::Dynamics: {
                ...
                int minVel = static_cast<int>(MIDI_VELOCITY_SOFT - DYNAMICS_RULE_BREAK_VELOCITY_ADJUSTMENT + (1.0f - severity) * (MIDI_VELOCITY_SOFT - DYNAMICS_RULE_BREAK_VELOCITY_ADJUSTMENT));
                int maxVel = static_cast<int>(MIDI_VELOCITY_MEDIUM + DYNAMICS_RULE_BREAK_VELOCITY_ADJUSTMENT + severity * (MIDI_VELOCITY_MAX - MIDI_VELOCITY_MEDIUM - DYNAMICS_RULE_BREAK_VELOCITY_ADJUSTMENT));
                ...
                if (severity > RULE_BREAK_MODERATE) {
                    for (auto& note : midi.melody) {
                        ...
                        if (dist(rng_) < severity * CHROMATICISM_PROBABILITY_FACTOR) {
                            note.pitch += (dist(rng_) < 0.5f ? -INTERVAL_MINOR_SECOND : INTERVAL_MINOR_SECOND); // Chromatic shift
...
void MidiGenerator::applyVariations(
    GeneratedMidi& midi,
    const IntentResult& intent,
    float complexity
) {
    ...
    if (!midi.bass.empty() && complexity > BASS_VARIATION_THRESHOLD) {
        ...
        VariationOutput output = variationEngine_.generate(config);
        if (complexity > BASS_VARIATION_REPLACE_THRESHOLD && !output.notes.empty()) {
            midi.bass = output.notes;
        }
    }

    // Apply variations to counter melody (if exists)
    if (!midi.counterMelody.empty() && complexity > COUNTER_MELODY_VARIATION_THRESHOLD) {
        VariationConfig config;
        config.emotion = getEmotionName(intent.emotion);
        config.source = midi.counterMelody;
        config.key = key;
        config.mode = mode;
        config.intensity = complexity * COUNTER_MELODY_VARIATION_INTENSITY_MULTIPLIER;
        config.preserveContour = true;
        config.preserveRhythm = false;
        config.seed = rng_();

        VariationOutput output = variationEngine_.generate(config);
        if (complexity > COUNTER_MELODY_VARIATION_REPLACE_THRESHOLD && !output.notes.empty()) {
            midi.counterMelody = output.notes;
        }
    }
}
...
void MidiGenerator::applyGrooveAndHumanize(
    ...
    if (humanize < MIN_HUMANIZE_THRESHOLD && std::abs(feel) < MIN_HUMANIZE_THRESHOLD) return;

    // Determine groove type from emotion
    GrooveType grooveType = GrooveType::Straight;
    if (emotion.arousal > AROUSAL_HIGH) grooveType = GrooveType::Syncopated;
    else if (emotion.arousal < AROUSAL_LOW) grooveType = GrooveType::Shuffle;
    else if (emotion.valence > VALENCE_POSITIVE) grooveType = GrooveType::Swing;
    ...
    if (!midi.counterMelody.empty()) {
        midi.counterMelody = grooveEngine_.applyEmotionTiming(midi.counterMelody, emotion);
    }

    // Apply groove
    midi.melody = grooveEngine_.applyGroove(midi.melody, grooveType, humanize);
    midi.bass = grooveEngine_.applyGroove(midi.bass, grooveType, humanize * BASS_HUMANIZE_MULTIPLIER);
    if (!midi.counterMelody.empty()) {
        midi.counterMelody = grooveEngine_.applyGroove(midi.counterMelody, grooveType, humanize * COUNTER_MELODY_HUMANIZE_MULTIPLIER);
    }
    ...
    if (std::abs(feel) > MIN_HUMANIZE_THRESHOLD) {
        ...
        midi.bass = grooveEngine_.applyTimingFeel(midi.bass, feel, BASS_HUMANIZE_MULTIPLIER);
        if (!midi.counterMelody.empty()) {
            midi.counterMelody = grooveEngine_.applyTimingFeel(midi.counterMelody, feel, COUNTER_MELODY_HUMANIZE_MULTIPLIER);
        }
    }
}
...
std::vector<std::string> MidiGenerator::chordsToStrings(const std::vector<Chord>& chords) {
...
double MidiGenerator::ticksToBeats(int ticks, int tempoBpm) {
    juce::ignoreUnused(tempoBpm); // Tempo doesn't affect ticks-to-beats conversion
    return static_cast<double>(ticks) / MIDI_PPQ;
}

int MidiGenerator::beatsToTicks(double beats, int tempoBpm) {
    juce::ignoreUnused(tempoBpm); // Tempo doesn't affect beats-to-ticks conversion
...
MidiNote MidiGenerator::counterMelodyNoteToMidi(const CounterMelodyNote& note, int tempoBpm) {
    MidiNote midiNote;
    midiNote.pitch = note.pitch;
    midiNote.velocity = note.velocity;
    midiNote.startBeat = ticksToBeats(note.startTick, tempoBpm);
...
MidiNote MidiGenerator::transitionNoteToMidi(const TransitionNote& note, int tempoBpm) {
    MidiNote midiNote;
    midiNote.pitch = note.pitch;
    midiNote.velocity = note.velocity;
    midiNote.startBeat = ticksToBeats(note.startTick, tempoBpm);
...
MidiGenerator::LayerFlags MidiGenerator::determineLayers(
    ...
    if (arrangement.has_value()) {
        ...
        flags.pads = complexity > PADS_COMPLEXITY_THRESHOLD || intent.emotion.arousal < AROUSAL_LOW;
        ...
        // Rhythm and drum groove: medium complexity, high arousal, or high-energy sections
        ...
        flags.pads = complexity > PADS_COMPLEXITY_THRESHOLD || intent.emotion.arousal < AROUSAL_LOW;
        flags.strings = complexity > INTENSITY_HIGH || intent.emotion.intensity > INTENSITY_VERY_HIGH;
        flags.counterMelody = complexity > INTENSITY_VERY_HIGH;
        flags.rhythm = complexity > RULE_BREAK_MODERATE || intent.emotion.arousal > AROUSAL_HIGH;
        flags.drumGroove = complexity > RULE_BREAK_MODERATE || intent.emotion.arousal > AROUSAL_HIGH;
        flags.fills = complexity > RULE_BREAK_MODERATE;
