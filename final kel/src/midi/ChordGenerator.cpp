...
void ChordGenerator::initializeProgressionFamilies() {
    ...
    families_.push_back({
        "I-IV-V-I",
        "universal",
        {INTERVAL_UNISON, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_UNISON},
        {"I", "IV", "V", "I"},
    ...
    families_.push_back({
        "I-V-vi-IV",
        "universal",
        {INTERVAL_UNISON, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH, INTERVAL_PERFECT_FOURTH},
        {"I", "V", "vi", "IV"},
    ...
    families_.push_back({
        "I-vi-IV-V",
        "universal",
        {INTERVAL_UNISON, INTERVAL_MAJOR_SIXTH, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH},
        {"I", "vi", "IV", "V"},
        {0.3f, 0.7f},  // Positive
    ...
    families_.push_back({
        "vi-IV-I-V",
        "universal",
        {INTERVAL_MAJOR_SIXTH, INTERVAL_PERFECT_FOURTH, INTERVAL_UNISON, INTERVAL_PERFECT_FIFTH},
        {"vi", "IV", "I", "V"},
    ...
    families_.push_back({
        "I-vi-ii-V",
        "jazz",
        {INTERVAL_UNISON, INTERVAL_MAJOR_SIXTH, INTERVAL_MAJOR_SECOND, INTERVAL_PERFECT_FIFTH},
        {"I", "vi", "ii", "V"},
        {0.3f, 0.8f},
        {0.5f, 0.9f},
        "Bouncy, classic jazz",
    ...
    families_.push_back({
        "iii-vi-ii-V",
        "jazz",
        {INTERVAL_MAJOR_THIRD, INTERVAL_MAJOR_SIXTH, INTERVAL_MAJOR_SECOND, INTERVAL_PERFECT_FIFTH},
        {"iii", "vi", "ii", "V"},
        {0.5f, 0.9f},
        {0.4f, 0.7f},
        "Elegant, flowing",
    ...
    families_.push_back({
        "i-bVI-bIII-bVII",
        "minor",
        {INTERVAL_UNISON, INTERVAL_MINOR_SIXTH, INTERVAL_MINOR_THIRD, INTERVAL_MINOR_SEVENTH},
        {"i", "bVI", "bIII", "bVII"},
    ...
    families_.push_back({
        "i-bVII-bVI-bVII",
        "minor",
        {INTERVAL_UNISON, INTERVAL_MINOR_SEVENTH, INTERVAL_MINOR_SIXTH, INTERVAL_MINOR_SEVENTH},
        {"i", "bVII", "bVI", "bVII"},
        {-0.7f, -0.1f},
        {0.3f, 0.7f},
    ...
    families_.push_back({
        "12-bar-blues",
        "blues",
        {INTERVAL_UNISON, INTERVAL_UNISON, INTERVAL_UNISON, INTERVAL_UNISON, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FOURTH, INTERVAL_UNISON, INTERVAL_UNISON, INTERVAL_PERFECT_FIFTH, INTERVAL_PERFECT_FOURTH, INTERVAL_UNISON, INTERVAL_PERFECT_FIFTH},
...
std::vector<Chord> ChordGenerator::generate(const IntentResult& intent, int bars) {
    ...
    const ProgressionFamily* family = selectFamilyForEmotion(valence, arousal, mode);
    ...
    if (!family) {
        ...
        fallbackFamily.useExtensions = intensity > CHORD_EXTENSION_INTENSITY_THRESHOLD;
    ...
    bool addExtensions = family->useExtensions || intensity > CHORD_EXTENSION_INTENSITY_THRESHOLD;

    for (size_t i = 0; i < family->degrees.size(); ++i) {
        int degree = family->degrees[i];
        Chord chord = buildChord(degree, mode, rootNote,
    ...
    if (shouldLoopProgression(bars, family->degrees)) {
        int loops = static_cast<int>(std::ceil((bars * BEATS_PER_BAR) / currentBeat)) - 1;
    ...
    for (const auto& rb : intent.ruleBreaks) {
        if (rb.type == RuleBreakType::Harmony) {
            if (rb.severity > RULE_BREAK_MODERATE) {  // Use severity as threshold for parallel motion
    ...
    for (const auto& rb : intent.ruleBreaks) {
        if (rb.type == RuleBreakType::Harmony) {
            ...
            if (rb.severity > INTENSITY_HIGH) {  // Use severity as threshold for dissonance
...
std::vector<Chord> ChordGenerator::generateProgression(
    ...
    intent.emotion.valence = (mode == "major" || mode == "lydian") ? VALENCE_POSITIVE : VALENCE_SLIGHTLY_NEGATIVE;
...
std::vector<Chord> ChordGenerator::generateFromFamily(
...
const ChordGenerator::ProgressionFamily* ChordGenerator::selectFamilyForEmotion(
    float valence,
    float arousal,
    const std::string& mode
) const {
...
float ChordGenerator::scoreFamilyForEmotion(
    ...
    // Add some randomness for variety (thread-safe: rng_ is protected by mutex in calling function)
...
std::vector<int> ChordGenerator::getScaleIntervals(const std::string& mode) const {
    if (mode == "major" || mode == "ionian") return {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MAJOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH, INTERVAL_MAJOR_SEVENTH};
    if (mode == "minor" || mode == "aeolian") return {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MINOR_SIXTH, INTERVAL_MINOR_SEVENTH};
    if (mode == "dorian") return {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH, INTERVAL_MINOR_SEVENTH};
    if (mode == "phrygian") return {INTERVAL_UNISON, INTERVAL_MINOR_SECOND, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MINOR_SIXTH, INTERVAL_MINOR_SEVENTH};
    if (mode == "lydian") return {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MAJOR_THIRD, INTERVAL_TRITONE, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH, INTERVAL_MAJOR_SEVENTH};
    if (mode == "mixolydian") return {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MAJOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH, INTERVAL_MINOR_SEVENTH};
    if (mode == "locrian") return {INTERVAL_UNISON, INTERVAL_MINOR_SECOND, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_TRITONE, INTERVAL_MINOR_SIXTH, INTERVAL_MINOR_SEVENTH};

    return {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MINOR_SIXTH, INTERVAL_MINOR_SEVENTH};  // Default to natural minor
}

int ChordGenerator::degreeToMidiNote(
    int degree,
    const std::string& mode,
    int rootNote,
    int octave
...
Chord ChordGenerator::buildChord(
    int degree,
    const std::string& mode,
    int rootNote,
    double startBeat,
    ...
    while (third < chordRoot) third += INTERVAL_OCTAVE;
    while (fifth < third) fifth += INTERVAL_OCTAVE;
    ...
    if (thirdInterval == INTERVAL_MINOR_THIRD) chord.name += "m";  // Minor third
    if (addExtension) chord.name += "7";

    return chord;
}

void ChordGenerator::addExtensions(
    Chord& chord,
    int degree,
    const std::string& mode,
    float intensity
) {
    if (chord.pitches.empty()) return;
    ...
    while (seventh < chord.pitches.back()) seventh += INTERVAL_OCTAVE;
    ...
    if (intensity > INTENSITY_HIGH) {
        ...
        while (ninth < chord.pitches.back()) ninth += INTERVAL_OCTAVE;
...
std::vector<Chord> ChordGenerator::applyVoiceLeading(
...
void ChordGenerator::applyVoiceLeadingToProgression(
    std::vector<Chord>& chords,
    VoiceLeadingStyle style
) {
    ...
    std::vector<std::vector<int>> voicedVoicings =
        voiceLeadingEngine_->voiceProgression(voicings, voicings[0].empty() ? DEFAULT_BASS : voicings[0][0]);
...
std::vector<int> ChordGenerator::chordToVoicing(
    const Chord& chord,
    int bassNote,
    int numVoices
) const {
    ...
    constexpr int MIN_VOICE_SPACING = INTERVAL_MINOR_THIRD;  // Minimum spacing between voices
    ...
    while (voicing.size() < static_cast<size_t>(numVoices) && !sortedPitches.empty()) {
        int lastPitch = voicing.back();
        int nextPitch = sortedPitches[voicing.size() % sortedPitches.size()];
        while (nextPitch <= lastPitch) nextPitch += INTERVAL_OCTAVE;
...
void ChordGenerator::addChromaticism(std::vector<Chord>& chords, float severity) {
    ...
    for (size_t i = 0; i < chords.size(); ++i) {
        ...
        if (i < chords.size() - 1 && chance(rng_) < severity * CHROMATIC_PASSING_PROBABILITY) {
            Chord passing = chords[i];

            // Shift all notes up or down by half step toward next chord
            int nextRoot = chords[i + 1].pitches.empty() ? 0 : chords[i + 1].pitches[0];
...
void ChordGenerator::applyModalInterchange(
    std::vector<Chord>& chords,
...
std::string ChordGenerator::generateChordName(
    const std::vector<int>& pitches,
    int rootNote
) const {
    if (pitches.empty()) return "N.C.";
    ...
    if (pitches.size() >= 2) {
        ...
        if (third == INTERVAL_MINOR_THIRD) name += "m";  // Minor
    }

    if (pitches.size() >= 3) {
        int fifth = pitches[2] - pitches[0];
        if (fifth == INTERVAL_TRITONE) name += "b5";  // Diminished
    }

    return name;
}

bool ChordGenerator::shouldLoopProgression(int bars, const std::vector<int>& degrees) const {
