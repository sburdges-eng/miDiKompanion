...
PluginEditor::PluginEditor(PluginProcessor& p)
    ...
    // User requirement: 400x300, 600x450, 800x600 (but we need more height for all controls)
    ...
    processor_.getAPVTS().addParameterListener(PluginProcessor::PARAM_VALENCE, this);
    processor_.getAPVTS().addParameterListener(PluginProcessor::PARAM_AROUSAL, this);
    processor_.getAPVTS().addParameterListener(PluginProcessor::PARAM_INTENSITY, this);
    processor_.getAPVTS().addParameterListener(PluginProcessor::PARAM_COMPLEXITY, this);
    processor_.getAPVTS().addParameterListener(PluginProcessor::PARAM_HUMANIZE, this);
    processor_.getAPVTS().addParameterListener(PluginProcessor::PARAM_FEEL, this);
    processor_.getAPVTS().addParameterListener(PluginProcessor::PARAM_DYNAMICS, this);
    ...
    if (processor_.hasPendingMidi()) {
        ...
        if (!generatedMidi.chords.empty() || !generatedMidi.melody.empty()) {
            ...
            if (!generatedMidi.chords.empty()) {
                ...
                workstation_->getChordDisplay().setChord(
                    juce::String(firstChord.name),
                    firstChord.pitches
                );
            }
        }
    ...
    float valence = *processor_.getAPVTS().getRawParameterValue(PluginProcessor::PARAM_VALENCE);
    float arousal = *processor_.getAPVTS().getRawParameterValue(PluginProcessor::PARAM_AROUSAL);
    float intensity = *processor_.getAPVTS().getRawParameterValue(PluginProcessor::PARAM_INTENSITY);
    ...
    // 100ms = 10 FPS for general updates (workstation has its own 30ms timer for animations)
    startTimer(100);
...
PluginEditor::~PluginEditor() {
    ...
    processor_.getAPVTS().removeParameterListener(PluginProcessor::PARAM_VALENCE, this);
    processor_.getAPVTS().removeParameterListener(PluginProcessor::PARAM_AROUSAL, this);
    processor_.getAPVTS().removeParameterListener(PluginProcessor::PARAM_INTENSITY, this);
    processor_.getAPVTS().removeParameterListener(PluginProcessor::PARAM_COMPLEXITY, this);
    processor_.getAPVTS().removeParameterListener(PluginProcessor::PARAM_HUMANIZE, this);
    processor_.getAPVTS().removeParameterListener(PluginProcessor::PARAM_FEEL, this);
    processor_.getAPVTS().removeParameterListener(PluginProcessor::PARAM_DYNAMICS, this);
    processor_.getAPVTS().removeParameterListener(PluginProcessor::PARAM_BARS, this);
...
void PluginEditor::timerCallback() {
    ...
    // This provides indication that regeneration is needed (manual regeneration approach)
    if (processor_.hasParametersChanged()) {
        ...
        // Otherwise, the button's normal state is sufficient (user will click when ready)
    ...
    // ========================================================================
    // Update EmotionRadar with current parameter values for real-time visualization
    // Note: This is also updated via parameterChanged() callback, but we ensure
    // it's always in sync here as a backup
    float valence = *processor_.getAPVTS().getRawParameterValue(PluginProcessor::PARAM_VALENCE);
    float arousal = *processor_.getAPVTS().getRawParameterValue(PluginProcessor::PARAM_AROUSAL);
    float intensity = *processor_.getAPVTS().getRawParameterValue(PluginProcessor::PARAM_INTENSITY);
    workstation_->getEmotionRadar().setEmotion(valence, arousal, intensity);
...
void PluginEditor::onGenerateClicked() {
    ...
    if (processor_.isGenerating()) {
        juce::AlertWindow::showMessageBoxAsync(
            ...
            "MIDI generation is already in progress. Please wait for it to complete.",
            "OK"
        );
        return;
    }
    ...
    if (woundText.isNotEmpty() && woundText.length() > 1000) {
        ...
        juce::AlertWindow::showMessageBoxAsync(
            ...
            "Invalid Input",
            "Wound text is too long. Please limit to 1000 characters.",
            "OK"
        );
        return;
    }
    ...
    processor_.setMusicTheorySettings(
        theorySettings.key,
        theorySettings.mode,
    ...
    if (woundText.isNotEmpty()) {
        ...
        try {
            ...
            wound.intensity = *processor_.getAPVTS().getRawParameterValue(PluginProcessor::PARAM_INTENSITY);
            wound.source = "user_input";

            // Process wound to get emotion (this updates parameters)
            // Note: This is a preview - actual generation will use the same processing
            ...
            workstation_->getEmotionRadar().setEmotion(
                previewIntent.emotion.valence,
                previewIntent.emotion.arousal,
    ...
    try {
        ...
        int checkCount = 0;
        const int maxChecks = 100;  // Safety limit (shouldn't be needed for sync generation)
        ...
        juce::AlertWindow::showMessageBoxAsync(
            juce::AlertWindow::WarningIcon,
            "Generation Failed",
            "An unknown error occurred while generating MIDI.",
            "OK"
        );
    }
...
void PluginEditor::onExportClicked() {
    ...
    bool isStandalone = (processor_.wrapperType == juce::AudioProcessor::wrapperType_Standalone);
    ...
    if (isStandalone) {
        ...
        if (generatedMidi.chords.empty() && generatedMidi.melody.empty() && generatedMidi.bass.empty()) {
            juce::AlertWindow::showMessageBoxAsync(
                juce::AlertWindow::WarningIcon,
                "No MIDI to Export",
                "Please generate MIDI first by clicking the Generate button.",
                "OK"
            );
            return;
        }
        ...
        chooser->launchAsync(
            ...
            [this, chooser](const juce::FileChooser& fc) {
                ...
                if (file != juce::File{}) {
                    ...
                    // Build MIDI file from generated MIDI data
                    MidiBuilder builder;
                    juce::MidiFile midiFile = builder.buildMidiFile(processor_.getGeneratedMidi());
...
void PluginEditor::onEmotionSelected(const EmotionNode& emotion) {
    ...
    //
    // Here we update processor and additional UI elements based on emotion selection
    ...
    // Use EmotionThesaurus to suggest mode (more accurate than simple valence check)
    ...
    // - Trust/Anticipation: Balanced instruments (String Ensemble, Acoustic Guitar)
    ...
    // Instrument selection could be added via MusicTheoryPanel setter methods if needed.
}
...
void PluginEditor::parameterChanged(const juce::String& parameterID, float newValue) {
    ...
    if (parameterID == PluginProcessor::PARAM_VALENCE ||
        ...
        const auto& thesaurus = processor_.getIntentPipeline().thesaurus();
        EmotionNode nearestEmotion = thesaurus.findNearest(valence, arousal, intensity);

        // If the nearest emotion is very close (within threshold), highlight it
        float distance = std::sqrt(
            std::pow(valence - nearestEmotion.valence, 2.0f) +
