...
// We don't include it directly to avoid type conflicts between Types.h and KellyTypes.h
// KellyBrain.h includes KellyTypes.h, while IntentPipeline.h includes Types.h
...
// Include these headers only in PluginProcessor.cpp where conversions are handled
namespace kelly {
    class KellyBrain;
...
public:
    ...
    // NOTE: Returns non-owning reference. The thesaurus is thread-safe, but process()/processJourney()
    // should only be called from UI thread. For audio thread access, use try_lock pattern.
    // IntentPipeline owns EmotionThesaurus, which is passed as non-owning pointer to VADSystem/VADCalculator.
    // Lifetime is guaranteed: IntentPipeline outlives all components that reference thesaurus.
    ...
    // Check if parameters have changed (for UI to show regeneration needed indicator)
    ...
    Kelly::ML::MultiModelProcessor& getMultiModelProcessor() { return multiModelProcessor_; }
    const Kelly::ML::MultiModelProcessor& getMultiModelProcessor() const { return multiModelProcessor_; }
    ...
    void setMusicTheorySettings(const juce::String& key, const juce::String& mode, int tempoBpm);
...
private:
    ...
    IntentPipeline intentPipeline_;  // Default pipeline (always available for backward compatibility)
    MidiGenerator midiGenerator_;
    MidiBuilder midiBuilder_;

    // Optional high-level APIs (using pointers to avoid including headers with conflicting types)
    std::unique_ptr<kelly::KellyBrain> kellyBrain_;  // High-level API wrapper (optional)
    std::unique_ptr<kelly::MLIntentPipeline> mlBridge_;      // ML-to-Intent bridge (optional)
    std::atomic<bool> useKellyBrainAPI_{false};  // Use KellyBrain instead of IntentPipeline
    std::atomic<bool> useMLBridge_{false};       // Use MLBridge for audio-driven generation
    ...
    std::atomic<bool> autoRegenerateEnabled_{false};  // Disabled by default for manual regeneration
    ...
    std::optional<EmotionNode> lastProcessedEmotion_;  // Last emotion processed from wound text
    ...
    mutable std::mutex intentMutex_;         // Protects intentPipeline_, woundDescription_, selectedEmotionId_
                                               // Audio thread: use try_lock (never block)
                                               // UI thread: use lock_guard (can block)
    ...
    std::array<float, 128> extractFeatures(const juce::AudioBuffer<float>& buffer, int startPos);
