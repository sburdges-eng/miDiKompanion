...
namespace kelly {
    ...
    // These will be shadowed when KellyTypes.h is included, but we can use them for conversions
...
namespace kelly {
    namespace {
        ...
        // Convert Types::EmotionNode to KellyTypes::EmotionNode (for IntentResult conversion)
        // Note: Since Types.h includes KellyTypes.h, they're the same types
...
juce::AudioProcessorValueTreeState::ParameterLayout PluginProcessor::createParameterLayout() {
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID{PARAM_VALENCE, PARAM_VERSION},
        "Valence",
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID{PARAM_AROUSAL, PARAM_VERSION},
        "Arousal",
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID{PARAM_INTENSITY, PARAM_VERSION},
        "Intensity",
        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f),
        INTENSITY_MODERATE,
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID{PARAM_COMPLEXITY, PARAM_VERSION},
        "Complexity",
        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f),
        RULE_BREAK_MODERATE,
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID{PARAM_HUMANIZE, PARAM_VERSION},
        "Humanize",
        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f),
        0.4f,
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID{PARAM_FEEL, PARAM_VERSION},
        "Feel",
        juce::NormalisableRange<float>(-1.0f, 1.0f, 0.01f),
        0.0f,
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        juce::ParameterID{PARAM_DYNAMICS, PARAM_VERSION},
        "Dynamics",
        juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f),
    ...
    params.push_back(std::make_unique<juce::AudioParameterInt>(
        juce::ParameterID{PARAM_BARS, PARAM_VERSION},
        "Bars",
        4, 32, 8,
    ...
    params.push_back(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{PARAM_BYPASS, PARAM_VERSION},
        "Bypass",
        false
    ));
...
void PluginProcessor::prepareToPlay(double sampleRate, int samplesPerBlock) {
    ...
    lookaheadSamples_ = PluginLatencyManager::msToSamples(ML_LOOKAHEAD_MS, sampleRate);
    ...
    if (!multiModelProcessor_.isInitialized()) {
        ...
        asyncMLPipeline_ = std::make_unique<Kelly::ML::AsyncMLPipeline>(
            multiModelProcessor_
        );
    }
}
...
void PluginProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages) {
    ...
    if (mlInferenceEnabled_.load() && numChannels > 0) {
        ...
        for (int ch = 0; ch < numChannels && ch < lookaheadBuffer_.getNumChannels(); ++ch) {
            const float* src = buffer.getReadPointer(ch);
            float* dst = lookaheadBuffer_.getWritePointer(ch);

            for (int i = 0; i < numSamples; ++i) {
                int writeIdx = (lookaheadWritePos_ + i) % lookaheadBuffer_.getNumSamples();
                dst[writeIdx] = src[i];
            }
        }

        // Extract features from lookahead buffer for ML inference
        std::array<float, 128> features = extractFeatures(lookaheadBuffer_, lookaheadReadPos_);
        ...
        for (int ch = 0; ch < numChannels && ch < lookaheadBuffer_.getNumChannels(); ++ch) {
            ...
            for (int i = 0; i < numSamples; ++i) {
                int readIdx = (lookaheadReadPos_ + i) % lookaheadBuffer_.getNumSamples();
                dst[i] = src[readIdx];
            }
        }

        // Update positions
        lookaheadWritePos_ = (lookaheadWritePos_ + numSamples) % lookaheadBuffer_.getNumSamples();
        lookaheadReadPos_ = (lookaheadReadPos_ + numSamples) % lookaheadBuffer_.getNumSamples();
    } else {
    ...
    float bpm = static_cast<float>(position->getBpm().orFallback(static_cast<double>(MusicConstants::TEMPO_DEFAULT)));
    ...
    // CRITICAL: Audio thread must NEVER block. Use try_lock instead of lock_guard.
    // If we can't acquire the lock immediately, skip this block to avoid audio glitches.
    if (hasPendingMidi_.load() && isPlaying) {
        ...
        // Calculate current position in beats (PPQ is quarter notes, so divide by BEATS_PER_QUARTER_NOTE)
        ...
        auto scheduleNoteEvent = [&](int channel, int pitch, int velocity, double beatPosition, bool isNoteOn) {
            if (beatPosition >= blockStartBeat && beatPosition < blockEndBeat) {
                int sampleOffset = static_cast<int>((beatPosition - blockStartBeat) * samplesPerBeat);
                sampleOffset = juce::jlimit(0, numSamples - 1, sampleOffset);

                if (isNoteOn) {
                    midiMessages.addEvent(
                        juce::MidiMessage::noteOn(channel, pitch, static_cast<juce::uint8>(velocity)),
                        sampleOffset
                    );
                } else {
                    midiMessages.addEvent(
        ...
        for (const auto& chord : generatedMidi_.chords) {
            ...
            if (chordEndBeat > blockStartBeat && chordStartBeat < blockEndBeat) {
                ...
                if (chordStartBeat >= blockStartBeat && chordStartBeat < blockEndBeat) {
                    int sampleOffset = static_cast<int>((chordStartBeat - blockStartBeat) * samplesPerBeat);
                    sampleOffset = juce::jlimit(0, numSamples - 1, sampleOffset);

                    for (int pitch : chord.pitches) {
                        midiMessages.addEvent(
                            juce::MidiMessage::noteOn(MIDI_CHANNEL_CHORDS + 1, pitch,
                ...
                if (chordEndBeat >= blockStartBeat && chordEndBeat < blockEndBeat) {
                    int sampleOffset = static_cast<int>((chordEndBeat - blockStartBeat) * samplesPerBeat);
                    sampleOffset = juce::jlimit(0, numSamples - 1, sampleOffset);
        ...
        for (const auto& note : generatedMidi_.melody) {
            ...
            double noteEndBeat = (note.startTick + note.durationTicks) / TICKS_PER_BEAT;

            if (noteEndBeat > blockStartBeat && noteStartBeat < blockEndBeat) {
                scheduleNoteEvent(MIDI_CHANNEL_MELODY + 1, note.pitch, note.velocity, noteStartBeat, true);
                scheduleNoteEvent(MIDI_CHANNEL_MELODY + 1, note.pitch, 0, noteEndBeat, false);
            }
        }
        ...
        for (const auto& note : generatedMidi_.bass) {
            ...
            if (noteEndBeat > blockStartBeat && noteStartBeat < blockEndBeat) {
                scheduleNoteEvent(MIDI_CHANNEL_BASS + 1, note.pitch, note.velocity, noteStartBeat, true);
                scheduleNoteEvent(MIDI_CHANNEL_BASS + 1, note.pitch, 0, noteEndBeat, false);
            }
        }
        ...
        for (const auto& note : generatedMidi_.counterMelody) {
            ...
            if (noteEndBeat > blockStartBeat && noteStartBeat < blockEndBeat) {
                scheduleNoteEvent(MIDI_CHANNEL_COUNTER_MELODY + 1, note.pitch, note.velocity, noteStartBeat, true);
                scheduleNoteEvent(MIDI_CHANNEL_COUNTER_MELODY + 1, note.pitch, 0, noteEndBeat, false);
            }
        }
        ...
        for (const auto& note : generatedMidi_.pad) {
            ...
            if (noteEndBeat > blockStartBeat && noteStartBeat < blockEndBeat) {
                scheduleNoteEvent(MIDI_CHANNEL_PAD + 1, note.pitch, note.velocity, noteStartBeat, true);
                scheduleNoteEvent(MIDI_CHANNEL_PAD + 1, note.pitch, 0, noteEndBeat, false);
            }
        }
        ...
        for (const auto& note : generatedMidi_.strings) {
            ...
            if (noteEndBeat > blockStartBeat && noteStartBeat < blockEndBeat) {
                scheduleNoteEvent(MIDI_CHANNEL_STRINGS + 1, note.pitch, note.velocity, noteStartBeat, true);
                scheduleNoteEvent(MIDI_CHANNEL_STRINGS + 1, note.pitch, 0, noteEndBeat, false);
            }
        }
        ...
        for (const auto& note : generatedMidi_.fills) {
            ...
            if (noteEndBeat > blockStartBeat && noteStartBeat < blockEndBeat) {
                scheduleNoteEvent(MIDI_CHANNEL_FILLS + 1, note.pitch, note.velocity, noteStartBeat, true);
                scheduleNoteEvent(MIDI_CHANNEL_FILLS + 1, note.pitch, 0, noteEndBeat, false);
            }
        }
...
void PluginProcessor::getStateInformation(juce::MemoryBlock& destData) {
    ...
    auto state = pluginState_.saveState(apvts_, woundDescription_, selectedEmotionId_, cassetteState);
...
void PluginProcessor::setStateInformation(const void* data, int sizeInBytes) {
    ...
    if (xmlState && xmlState->hasTagName(apvts_.state.getType())) {
        ...
        std::lock_guard<std::mutex> lock(intentMutex_);
        CassetteState cassetteState;
        pluginState_.loadState(apvts_, woundDescription_, selectedEmotionId_, cassetteState, state);
...
void PluginProcessor::generateMidi() {
    ...
    EmotionNode processedEmotion;
    {
        ...
        if (useKellyBrainAPI_.load() && kellyBrain_) {
            ...
            if (emotionId) {
                ...
                auto emotionOpt = kellyBrain_->pipeline().thesaurus().findById(*emotionId);
                if (emotionOpt.has_value()) {
                    processedEmotion = *emotionOpt;
                    // Set emotion in wound
                    kellyWound.primaryEmotion = convertLegacyToKellyTypesEmotionNode(processedEmotion);
                    ...
                    // Use fromJourney() method on KellyBrain, not pipeline().processJourney()
                    ::kelly::IntentResult kellyIntent = kellyBrain_->fromJourney(sideA, sideB);
                    // Convert KellyTypes::IntentResult to Types::IntentResult
                ...
                EmotionNode nearestEmotion = kellyBrain_->pipeline().thesaurus().findNearest(valence, arousal, intensity);
        ...
        // Option 2: Use existing IntentPipeline (default, backward compatible)
            ...
            if (emotionId) {
                ...
                if (emotionOpt.has_value()) {
                    ...
                    // This handles cases where emotion ID might be invalid or thesaurus not fully loaded
                ...
                intent = intentPipeline_.process(wound);
                processedEmotion = intent.emotion;  // Store the detected emotion from wound text
                ...
                // 3. Parameters can be updated by the UI if user wants to match detected emotion
            } else {
                // Use valence/arousal/intensity coordinates
                EmotionNode nearestEmotion = intentPipeline_.thesaurus().findNearest(valence, arousal, intensity);
                processedEmotion = nearestEmotion;
    ...
    // Note: Full integration may require API changes to IntentPipeline/MidiGenerator
    // For now, we store the settings and they can be used in future enhancements
...
void PluginProcessor::setMusicTheorySettings(const juce::String& key, const juce::String& mode, int tempoBpm) {
    // Called from UI thread - safe to lock
    std::lock_guard<std::mutex> lock(intentMutex_);
...
PluginProcessor::MusicTheorySettings PluginProcessor::getMusicTheorySettings() const {
    // Called from UI thread - safe to lock
    std::lock_guard<std::mutex> lock(intentMutex_);
...
void PluginProcessor::enableMLInference(bool enable) {
    ...
    if (enable) {
        ...
        juce::File modelFile = juce::File::getSpecialLocation(
            juce::File::currentApplicationFile
        ...
        if (modelFile.existsAsFile()) {
            ...
            juce::Logger::writeToLog("ML Inference enabled but model file not found: " +
    ...
    if (enable) {
        ...
        if (!kellyBrain_) {
            ...
            if (!dataDir.isDirectory()) {
                dataDir = juce::File::getSpecialLocation(
                    juce::File::currentApplicationFile
            ...
            if (dataDir.isDirectory()) {
                ...
                juce::Logger::writeToLog("KellyBrain enabled but data directory not found: " +
...
std::array<float, 128> PluginProcessor::extractFeatures(
...
void PluginProcessor::applyEmotionVector(const std::array<float, 64>& emotionVector) {
    ...
    float arousal = (std::tanh(arousalSum / 32.0f) + 1.0f) * 0.5f;  // Map to [0, 1]
...
void PluginProcessor::parameterChanged(const juce::String& parameterID, float newValue) {
    // This is called from the message thread when parameters change (including automation)
    // Parameters are already stored in APVTS atomically, so they're safe to read from audio thread

    juce::ignoreUnused(newValue);

    // Manual regeneration approach: Track parameter changes but don't auto-regenerate
    // User requested manual regeneration only (no auto-regeneration during playback)
    // The UI will show visual feedback when regeneration is needed
    ...
    // Mark that parameters have changed (for UI to show regeneration needed indicator)
    ...
    if (autoRegenerateEnabled_.load()) {
        // Real-time regeneration: Only regenerate when host is playing and auto-regeneration is enabled
        if (!isHostPlaying_.load()) {
    ...
    if (enable) {
        ...
        if (!mlBridge_) {
            ...
            std::string modelsPath = modelsDir.isDirectory()
                ...
                : "./models";
            std::string dataPath = dataDir.isDirectory()
                ? dataDir.getFullPathName().toStdString()
                : "";
...
kelly::MLIntentPipeline& PluginProcessor::getMLBridge() {
    return *mlBridge_;
}
