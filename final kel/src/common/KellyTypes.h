...
struct Wound;
struct IntentResult;
struct ArrangementOutput;  // Forward declaration (defined in engines/ArrangementEngine.h)
...
struct GeneratedMidi {
    ...
    // Use pointer to avoid requiring complete type definition (avoids circular dependency)
    ArrangementOutput* arrangement = nullptr;  // Section structure metadata (optional, null if not set)
};
...
struct Wound {
    ...
    float intensity = 0.5f;          // Alias for urgency (for compatibility with existing code)
};
...
struct IntentResult {
    ...
    EmotionNode emotion;             // Emotion associated with intent (should be synced with sourceWound.primaryEmotion)
...
struct DrumHit {
    ...
    // Getter for backwards compatibility (some code uses .tick instead of .startTick)
...
inline int noteNameToMidi(const std::string& name) {
    ...
    size_t i = 1;
    if (name.size() > 1 && (name[1] == '#' || name[1] == 'b')) i = 2;
...
inline const char* ruleBreakToString(RuleBreakType type) {
    static const char* names[] = {
        "None", "Harmony", "Rhythm", "Dynamics", "Melody", "Form",
        "ModalMixture", "ParallelMotion", "UnresolvedTension",
        "CrossRhythm", "RegisterShift", "DynamicContrast", "HarmonicAmbiguity"
    };
