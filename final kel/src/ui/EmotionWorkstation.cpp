...
EmotionWorkstation::EmotionWorkstation(juce::AudioProcessorValueTreeState& apvts)
    : apvts_(apvts) {
    setupComponents();
}
...
void EmotionWorkstation::setupComponents() {
    ...
    woundInput_.setTextToShowWhenEmpty("Describe what you're feeling...",
                                       lookAndFeel_.textSecondary.withAlpha(0.6f));
    woundInput_.setFont(juce::Font(14.0f));
    woundInput_.setTitle("Wound input field");
    woundInput_.setDescription("Describe your emotional state or wound for processing");
    ...
    emotionWheel_.setDescription("216-node emotion selector - click to select an emotion");
    ...
    valenceSlider_.setDescription("Emotional valence: negative (sad/angry) to positive (happy/joyful)");
    ...
    arousalSlider_.setDescription("Emotional arousal: calm/low energy to excited/high energy");
    ...
    intensitySlider_.setDescription("Emotional intensity: subtle feeling to overwhelming emotion");
    ...
    complexitySlider_.setDescription("Musical complexity: simple patterns to complex arrangements");
    ...
    humanizeSlider_.setDescription("Humanization: perfect timing to natural human feel");

    setupSlider(feelSlider_, feelLabel_, "Feel");
    feelSlider_.setRange(-1.0, 1.0, 0.01);
    feelSlider_.setTextValueSuffix(" (laid back ← → pushed)");
    feelSlider_.setDescription("Timing feel: laid back/behind the beat to pushed/ahead of the beat");
    ...
    dynamicsSlider_.setDescription("Dynamic range: soft/quiet to loud/intense velocities");
    ...
    bypassButton_.setDescription("Bypass Kelly MIDI generation and pass through input MIDI");
    ...
    arousalAttachment_ = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
        apvts_, PluginProcessor::PARAM_AROUSAL, arousalSlider_);
    intensityAttachment_ = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
        apvts_, PluginProcessor::PARAM_INTENSITY, intensitySlider_);
    complexityAttachment_ = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
        apvts_, PluginProcessor::PARAM_COMPLEXITY, complexitySlider_);
    humanizeAttachment_ = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
        apvts_, PluginProcessor::PARAM_HUMANIZE, humanizeSlider_);
    feelAttachment_ = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
        apvts_, PluginProcessor::PARAM_FEEL, feelSlider_);
    dynamicsAttachment_ = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
        apvts_, PluginProcessor::PARAM_DYNAMICS, dynamicsSlider_);
    barsAttachment_ = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
    ...
    musicTheoryPanel_.setDescription("Music theory settings: key, mode, tempo, instruments");
    ...
    generateButton_.setDescription("Generate MIDI from current emotional parameters");
    ...
    exportButton_.setDescription("Export generated MIDI to your digital audio workstation");
    ...
    lyricDisplay_.setDescription("Display generated lyrics with syllable breakdown");
...
void EmotionWorkstation::setupButton(juce::Button& button, const juce::String& tooltip) {
    button.setTooltip(tooltip);
    addAndMakeVisible(button);
}

void EmotionWorkstation::handleEmotionWheelSelection(const EmotionNode& emotion) {
...
void EmotionWorkstation::paint(juce::Graphics& g) {
    ...
    g.drawText("EMOTION MAPPING",
               bounds.getX() + 10,
               bounds.getY() + woundHeight + 10,
               bounds.getWidth() / 2 - 20,
               20,
               juce::Justification::centredLeft);

    // "MUSICAL PARAMETERS" header
    g.drawText("MUSICAL PARAMETERS",
               bounds.getX() + bounds.getWidth() / 2 + 10,
               bounds.getY() + woundHeight + 10,
               bounds.getWidth() / 2 - 20,
               20,
               juce::Justification::centredLeft);
    ...
    g.drawHorizontalLine(woundHeight + topSectionHeight + theoryHeight, 0.0f, (float)getWidth());
    g.drawHorizontalLine(woundHeight + topSectionHeight + theoryHeight + lyricHeight, 0.0f, (float)getWidth());
    g.drawHorizontalLine(woundHeight + topSectionHeight + theoryHeight + lyricHeight + vocalControlHeight, 0.0f, (float)getWidth());
    g.drawHorizontalLine(woundHeight + topSectionHeight + theoryHeight + lyricHeight + vocalControlHeight + pianoRollHeight,
                        0.0f, (float)getWidth());
...
void EmotionWorkstation::resized() {
    ...
    auto emotionWheelArea = leftPanel.removeFromTop(leftPanel.getHeight() / 2).reduced(5);
    ...
    auto vocalControlArea = bounds.removeFromTop(vocalControlHeight).reduced(margin);
    ...
    actionsRow = actionsRow.withSizeKeepingCentre(
        buttonWidth * 4 + buttonGap * 3,
        buttonHeight
    );
