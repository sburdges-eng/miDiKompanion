...
namespace {
    ...
    const std::map<std::string, std::vector<int>> SCALE_INTERVALS = {
        ...
        {"ionian", {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MAJOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH, INTERVAL_MAJOR_SEVENTH}},
        ...
        {"aeolian", {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MINOR_SIXTH, INTERVAL_MINOR_SEVENTH}},
        ...
        {"locrian", {INTERVAL_UNISON, INTERVAL_MINOR_SECOND, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_TRITONE, INTERVAL_MINOR_SIXTH, INTERVAL_MINOR_SEVENTH}},
        {"pentatonic_major", {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MAJOR_THIRD, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH}},
        {"pentatonic_minor", {INTERVAL_UNISON, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MINOR_SEVENTH}},
    ...
    int noteNameToMidi(const std::string& name, int octave = 4) {
        ...
        if (noteName.length() > 1 && (noteName.back() == 'm' || noteName.back() == 'M')) {
...
std::vector<int> MelodyEngine::getScalePitches(const std::string& key, const std::string& mode, int octave) {
...
std::vector<int> MelodyEngine::generateContour(ContourType contour, int numNotes, int startPitch, int range, std::mt19937& rng) {
    ...
    switch (contour) {
        case ContourType::Ascending:
            for (int i = 0; i < numNotes; ++i) {
                ...
                pitches[static_cast<size_t>(i)] = startPitch + static_cast<int>(progress * range);
            }
            break;
        ...
        case ContourType::Descending:
            for (int i = 0; i < numNotes; ++i) {
                float progress = static_cast<float>(i) / std::max(1, numNotes - 1);
                pitches[static_cast<size_t>(i)] = startPitch + range - static_cast<int>(progress * range);
            }
            break;
        ...
        case ContourType::Arch:
            for (int i = 0; i < numNotes; ++i) {
                ...
                pitches[static_cast<size_t>(i)] = startPitch + static_cast<int>(archValue * range);
            }
            break;
        ...
        case ContourType::InverseArch:
            for (int i = 0; i < numNotes; ++i) {
                float progress = static_cast<float>(i) / std::max(1, numNotes - 1);
                float archValue = static_cast<float>(-std::sin(progress * M_PI));
                pitches[static_cast<size_t>(i)] = startPitch + range / 2 + static_cast<int>(archValue * range / 2);
            }
            break;
        ...
        case ContourType::Static:
            for (int i = 0; i < numNotes; ++i) {
                ...
                int offset = static_cast<int>((dist(rng) - 0.5f) * STATIC_VARIANCE_SEMITONES);
        ...
        case ContourType::Wave:
            for (int i = 0; i < numNotes; ++i) {
                ...
                pitches[static_cast<size_t>(i)] = startPitch + range / 2 + static_cast<int>(waveValue * range / 3);
            }
            break;
        ...
        case ContourType::SpiralDown:
            for (size_t i = 0; i < static_cast<size_t>(numNotes); ++i) {
                ...
                float oscillation = static_cast<float>(std::sin(progress * M_PI * 4) * (static_cast<float>(range) / 6.0f) * (1.0f - progress));
                pitches[i] = startPitch + range - static_cast<int>(descent) + static_cast<int>(oscillation);
            }
            break;
        ...
        case ContourType::SpiralUp:
            for (size_t i = 0; i < static_cast<size_t>(numNotes); ++i) {
                ...
                float oscillation = static_cast<float>(std::sin(progress * M_PI * 4) * (static_cast<float>(range) / 6.0f) * (1.0f - progress));
                pitches[i] = startPitch + static_cast<int>(ascent) + static_cast<int>(oscillation);
            }
            break;
        ...
        case ContourType::Jagged:
            ...
            for (size_t i = 1; i < static_cast<size_t>(numNotes); ++i) {
                ...
                int jump = static_cast<int>((dist(rng) - 0.5f) * static_cast<float>(range) * JAGGED_JUMP_FACTOR);
                pitches[i] = std::clamp(pitches[i-1] + jump, startPitch, startPitch + range);
            }
            break;
        ...
        case ContourType::Collapse:
            for (size_t i = 0; i < static_cast<size_t>(numNotes); ++i) {
                ...
                if (progress < COLLAPSE_START_THRESHOLD) {
                    pitches[i] = startPitch + range - static_cast<int>(progress * static_cast<float>(range) * COLLAPSE_DESCENT_FACTOR);
                } else {
                    float collapse = (progress - COLLAPSE_START_THRESHOLD) / (1.0f - COLLAPSE_START_THRESHOLD);
                    pitches[i] = startPitch + static_cast<int>(static_cast<float>(range) * COLLAPSE_START_THRESHOLD * (1.0f - collapse));
                }
            }
            break;
    }
...
MelodyOutput MelodyEngine::generate(
...
MelodyOutput MelodyEngine::generate(const MelodyConfig& config) {
    ...
    RhythmDensity density = config.densityOverride.value_or(profile.preferredDensity);

    // Calculate notes per bar based on density
    int notesPerBar;
    switch (density) {
        case RhythmDensity::Sparse: notesPerBar = 2 + rng() % 3; break;
        case RhythmDensity::Moderate: notesPerBar = 4 + rng() % 5; break;
        case RhythmDensity::Dense: notesPerBar = 8 + rng() % 9; break;
        case RhythmDensity::Frantic: notesPerBar = 16 + rng() % 8; break;
    }
    ...
    // Get scale
    int baseOctave = (profile.registerRange.first + profile.registerRange.second) / 2 / INTERVAL_OCTAVE - 1;
    ...
    auto contourPitches = generateContour(contour, totalNotes, profile.registerRange.first, range, rng);

    // Generate notes
    std::vector<MelodyNote> notes;
    std::uniform_real_distribution<float> dist(0.0f, 1.0f);
    std::uniform_int_distribution<int> velDist(profile.velocityRange.first, profile.velocityRange.second);
    ...
    for (int i = 0; i < totalNotes; ++i) {
        ...
        if (useChords) {
            int chordIndex = getChordAtTick(startTick, totalTicks, config.chordProgression);
            if (chordIndex >= 0 && static_cast<size_t>(chordIndex) < config.chordProgression.size()) {
                auto chordTones = getChordTones(config.chordProgression[static_cast<size_t>(chordIndex)]);
                pitch = snapToChordOrScale(pitch, chordTones, scale, chordPreference, rng);
            } else {
        ...
        int duration = static_cast<int>(baseDuration * (profile.legatoRatio + dist(rng) * DURATION_VARIANCE));
        ...
        // Timing humanization
        int timingOffset = static_cast<int>((dist(rng) - 0.5f) * static_cast<float>(TIMING_OFFSET_MAX));

        notes.push_back({
            pitch, startTick, duration, velocity, art, timingOffset
        });
    }
...
MelodyOutput MelodyEngine::generateForSection(
    const std::string& emotion,
...
int MelodyEngine::getChordAtTick(int tick, int totalTicks, const std::vector<Chord>& chords) const {
    if (chords.empty()) return -1;

    // Convert tick to beat position
    double beatPosition = (static_cast<double>(tick) / TICKS_PER_BEAT) * BEATS_PER_BAR; // Assuming 4/4
...
std::vector<int> MelodyEngine::getChordTones(const Chord& chord) const {
    ...
    for (int pitch : chord.pitches) {
        ...
        for (int octave = 0; octave <= 2; ++octave) {
            ...
            if (tone >= MIDI_A0 && tone <= MIDI_PITCH_REASONABLE_MAX) {  // Reasonable MIDI range
...
int MelodyEngine::snapToChordOrScale(int pitch, const std::vector<int>& chordTones, const std::vector<int>& scaleTones, float chordPreference, std::mt19937& rng) const {
    std::uniform_real_distribution<float> dist(0.0f, 1.0f);
