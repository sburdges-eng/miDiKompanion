...
namespace {
    const int TICKS_PER_BEAT = MIDI_PPQ;
    // Removed static RNG - each generation creates its own from seed

    const std::map<std::string, std::vector<int>> SCALE_INTERVALS = {
        {"major", {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MAJOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH, INTERVAL_MAJOR_SEVENTH}},
        {"minor", {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MINOR_SIXTH, INTERVAL_MINOR_SEVENTH}},
        {"dorian", {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH, INTERVAL_MINOR_SEVENTH}},
        {"phrygian", {INTERVAL_UNISON, INTERVAL_MINOR_SECOND, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MINOR_SIXTH, INTERVAL_MINOR_SEVENTH}},
        {"lydian", {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MAJOR_THIRD, INTERVAL_TRITONE, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH, INTERVAL_MAJOR_SEVENTH}},
        {"mixolydian", {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MAJOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH, INTERVAL_MINOR_SEVENTH}},
        {"pentatonic", {INTERVAL_UNISON, INTERVAL_MAJOR_SECOND, INTERVAL_MAJOR_THIRD, INTERVAL_PERFECT_FIFTH, INTERVAL_MAJOR_SIXTH}},
        {"blues", {INTERVAL_UNISON, INTERVAL_MINOR_THIRD, INTERVAL_PERFECT_FOURTH, INTERVAL_TRITONE, INTERVAL_PERFECT_FIFTH, INTERVAL_MINOR_SEVENTH}},
    };
    ...
    int noteNameToSemitone(const std::string& name) {
        for (size_t i = 0; i < CHROMATIC.size(); ++i) {
            if (CHROMATIC[i] == name) return static_cast<int>(i);
        }
        return 0;
    }
}
...
void VariationEngine::initializeProfiles() {
    ...
    profiles_["grief"] = {
        {VariationType::Simplification, VariationType::Fragmentation, VariationType::Reduction},
        0.1f, 0.6f, 0.7f,
    ...
    profiles_["sadness"] = {
        {VariationType::Simplification, VariationType::Melodic, VariationType::Displacement},
        0.15f, 0.5f, 0.8f,
    ...
    profiles_["hope"] = {
        {VariationType::Extension, VariationType::Embellishment, VariationType::Melodic},
        0.4f, 0.2f, 1.1f,
    ...
    profiles_["joy"] = {
        {VariationType::Ornamentation, VariationType::Rhythmic, VariationType::Embellishment},
        0.6f, 0.1f, 1.2f,
        {OrnamentType::Trill, OrnamentType::Turn, OrnamentType::Mordent, OrnamentType::Acciaccatura}
    };

    // Anger - rhythmic, displacement
    profiles_["anger"] = {
        {VariationType::Rhythmic, VariationType::Displacement, VariationType::Fragmentation},
        0.3f, 0.2f, 1.3f,
    ...
    profiles_["fear"] = {
        {VariationType::Fragmentation, VariationType::Diminution, VariationType::Displacement},
        0.2f, 0.3f, 0.9f,
    ...
    profiles_["anxiety"] = {
        {VariationType::Rhythmic, VariationType::Diminution, VariationType::Displacement},
        0.25f, 0.25f, 1.1f,
    ...
    profiles_["peace"] = {
        {VariationType::Simplification, VariationType::Augmentation, VariationType::Reduction},
        0.1f, 0.7f, 0.6f,
        {OrnamentType::Slide}
    };
    ...
    profiles_["love"] = {
        {VariationType::Embellishment, VariationType::Extension, VariationType::Melodic},
        0.35f, 0.15f, 1.0f,
        {OrnamentType::Appoggiatura, OrnamentType::Turn, OrnamentType::GraceNote}
    };

    // Neutral
    profiles_["neutral"] = {
        {VariationType::Melodic, VariationType::Rhythmic},
        0.25f, 0.25f, 1.0f,
        {OrnamentType::Mordent, OrnamentType::Turn}
    };
}

std::vector<int> VariationEngine::getScalePitches(const std::string& key, const std::string& mode) {
    int root = noteNameToSemitone(key);
    auto it = SCALE_INTERVALS.find(mode);
    const auto& intervals = (it != SCALE_INTERVALS.end()) ? it->second : SCALE_INTERVALS.at("major");
...
float VariationEngine::calculateSimilarity(const std::vector<MidiNote>& a, const std::vector<MidiNote>& b) {
...
std::vector<MidiNote> VariationEngine::ornament(const std::vector<MidiNote>& source, OrnamentType type, std::mt19937& rng) {
        ...
        switch (type) {
            case OrnamentType::Trill: {
                ...
                for (int i = 0; i < trillCount; ++i) {
                    ...
                    result.push_back({pitch, note.velocity, startBeat, durBeats});
                }
                break;
            }

            case OrnamentType::Mordent: {
                double mordentDurBeats = static_cast<double>(TICKS_PER_BEAT / 16) / TICKS_PER_BEAT;
                result.push_back({note.pitch, note.velocity, note.startBeat, mordentDurBeats});
                result.push_back({note.pitch + 2, note.velocity - 10, note.startBeat + mordentDurBeats, mordentDurBeats});
                result.push_back({note.pitch, note.velocity, note.startBeat + mordentDurBeats * 2, note.duration - mordentDurBeats * 2});
                break;
            }

            case OrnamentType::Turn: {
                double turnDurBeats = static_cast<double>(TICKS_PER_BEAT / 8) / TICKS_PER_BEAT;
                result.push_back({note.pitch + 2, note.velocity - 5, note.startBeat, turnDurBeats});
                result.push_back({note.pitch, note.velocity, note.startBeat + turnDurBeats, turnDurBeats});
                result.push_back({note.pitch - 1, note.velocity - 5, note.startBeat + turnDurBeats * 2, turnDurBeats});
                result.push_back({note.pitch, note.velocity, note.startBeat + turnDurBeats * 3, note.duration - turnDurBeats * 3});
                break;
            }

            case OrnamentType::Appoggiatura: {
                double appoggDurBeats = note.duration / 4.0;
                result.push_back({note.pitch + 2, note.velocity, note.startBeat, appoggDurBeats});
                result.push_back({note.pitch, note.velocity, note.startBeat + appoggDurBeats, note.duration - appoggDurBeats});
                break;
            }

            case OrnamentType::Acciaccatura: {
                double graceDurBeats = static_cast<double>(TICKS_PER_BEAT / 32) / TICKS_PER_BEAT;
                result.push_back({note.pitch - 1, note.velocity - 15, note.startBeat, graceDurBeats});
                result.push_back({note.pitch, note.velocity, note.startBeat + graceDurBeats, note.duration});
                break;
            }

            case OrnamentType::GraceNote: {
                double graceDurBeats = static_cast<double>(TICKS_PER_BEAT / 16) / TICKS_PER_BEAT;
                result.push_back({note.pitch - 2, note.velocity - 10, note.startBeat - graceDurBeats, graceDurBeats});
...
std::vector<MidiNote> VariationEngine::simplify(const std::vector<MidiNote>& source, float amount, std::mt19937& rng) {
    ...
    for (size_t i = 0; i < source.size(); ++i) {
        ...
        if (isDownbeat) keepProb += 0.3f;
        if (isLong) keepProb += 0.2f;
        if (isFirst || isLast) keepProb += 0.4f;
    ...
    if (result.empty() && !source.empty()) {
        ...
        if (source.size() > 1) result.push_back(source.back());
    }

    return result;
}

std::vector<MidiNote> VariationEngine::rhythmicVariation(const std::vector<MidiNote>& source, float amount, std::mt19937& rng) {
    std::vector<MidiNote> result;
    std::uniform_real_distribution<float> dist(0.0f, 1.0f);
    std::uniform_int_distribution<int> offsetDist(-TICKS_PER_BEAT/4, TICKS_PER_BEAT/4);

    for (auto note : source) {
        if (dist(rng) < amount) {
            double offsetBeats = static_cast<double>(offsetDist(rng) * amount) / TICKS_PER_BEAT;
            ...
            if (dist(rng) < amount * 0.5f) {
                ...
                note.duration = std::max(static_cast<double>(TICKS_PER_BEAT/8) / TICKS_PER_BEAT,
...
std::vector<MidiNote> VariationEngine::transpose(const std::vector<MidiNote>& source, int semitones) {
    std::vector<MidiNote> result;
    for (auto note : source) {
...
std::vector<MidiNote> VariationEngine::invert(const std::vector<MidiNote>& source, int axis) {
    if (source.empty()) return source;
...
std::vector<MidiNote> VariationEngine::retrograde(const std::vector<MidiNote>& source) {
    if (source.empty()) return source;
...
std::vector<MidiNote> VariationEngine::augment(const std::vector<MidiNote>& source, float factor) {
    std::vector<MidiNote> result;
    for (auto note : source) {
...
std::vector<MidiNote> VariationEngine::diminish(const std::vector<MidiNote>& source, float factor) {
    return augment(source, 1.0f / factor);
}

std::vector<MidiNote> VariationEngine::fragment(const std::vector<MidiNote>& source, int fragments, std::mt19937& rng) {
    if (source.empty() || fragments <= 0) return source;

    std::vector<MidiNote> result;
    int notesPerFragment = static_cast<int>(source.size()) / fragments;
    if (notesPerFragment < 1) notesPerFragment = 1;

    // Pick random fragments
    for (int f = 0; f < fragments; ++f) {
        int startIdx = rng() % source.size();
        double totalDuration = source.back().startBeat + source.back().duration;
        for (int i = 0; i < notesPerFragment && startIdx + i < static_cast<int>(source.size()); ++i) {
...
std::vector<MidiNote> VariationEngine::extend(const std::vector<MidiNote>& source, int bars) {
    if (source.empty() || bars <= 0) return source;
...
std::vector<MidiNote> VariationEngine::displace(const std::vector<MidiNote>& source, int ticks) {
    std::vector<MidiNote> result;
...
VariationOutput VariationEngine::generate(
...
VariationOutput VariationEngine::generate(const VariationConfig& config) {
    ...
    switch (type) {
        ...
        case VariationType::Fragmentation:
            result = fragment(config.source, 2 + static_cast<int>(adjustedIntensity * 4), rng);
        ...
        case VariationType::Displacement:
            result = displace(config.source, static_cast<int>(TICKS_PER_BEAT * adjustedIntensity));
