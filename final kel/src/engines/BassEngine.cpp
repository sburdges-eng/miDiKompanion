...
int BassEngine::getRegisterOctave(BassRegister reg) {
    switch (reg) {
        case BassRegister::Sub: return 1;
        case BassRegister::Low: return 2;
        case BassRegister::Mid: return 3;
        case BassRegister::Flexible: return 2;  // Default to low for flexible
        default: return 2;
    }
}

BassEngine::ChordTones BassEngine::parseChord(const std::string& chord, int octave) {
    std::string root;
    std::string quality = "";
    ...
    ChordTones tones;
    tones.root = rootMidi;
    tones.third = rootMidi + (intervals.size() > 1 ? intervals[1] : INTERVAL_MAJOR_THIRD);
    tones.fifth = rootMidi + (intervals.size() > 2 ? intervals[2] : INTERVAL_PERFECT_FIFTH);
    tones.seventh = rootMidi + (intervals.size() > 3 ? intervals[3] : INTERVAL_MINOR_SEVENTH);

    return tones;
}

    std::vector<BassNote> BassEngine::generateRootOnly(const ChordTones& tones, int barTicks, const BassEmotionProfile& profile, std::mt19937& rng) {
    ...
    int duration = static_cast<int>(barTicks * profile.noteLengthRatio) - MIN_NOTE_DURATION_OFFSET;
    notes.push_back({tones.root, 0, duration, velDist(rng), BassArticulation::Sustained, "root"});

    return notes;
}

std::vector<BassNote> BassEngine::generateRootFifth(const ChordTones& tones, int barTicks, const BassEmotionProfile& profile, std::mt19937& rng) {
    std::vector<BassNote> notes;
    std::uniform_int_distribution<int> velDist(profile.velocityRange.first, profile.velocityRange.second);

    int halfBar = barTicks / 2;
    int duration = static_cast<int>(halfBar * profile.noteLengthRatio) - MIN_NOTE_DURATION_OFFSET;

    notes.push_back({tones.root, 0, duration, velDist(rng), BassArticulation::Sustained, "root"});
    notes.push_back({tones.fifth, halfBar, duration, velDist(rng), BassArticulation::Sustained, "fifth"});

    return notes;
}

std::vector<BassNote> BassEngine::generateWalking(const ChordTones& tones, int barTicks, const BassEmotionProfile& profile, std::mt19937& rng) {
    std::vector<BassNote> notes;
    std::uniform_int_distribution<int> velDist(profile.velocityRange.first, profile.velocityRange.second);
    ...
    int duration = static_cast<int>(beatTicks * profile.noteLengthRatio) - WALKING_NOTE_TAIL_OFFSET;

    int currentPitch = tones.root;
    constexpr int WALKING_PITCH_RANGE_BELOW = 5;
    constexpr int WALKING_PITCH_RANGE_ABOVE = 7;
    for (int i = 0; i < numBeats; ++i) {
        notes.push_back({currentPitch, i * beatTicks, duration, velDist(rng), BassArticulation::Sustained, "walk"});

        // Step toward next chord tone
        int step = stepDist(rng);
        if (rng() % 2 == 0) step = -step;
        currentPitch = std::clamp(currentPitch + step, tones.root - WALKING_PITCH_RANGE_BELOW, tones.root + WALKING_PITCH_RANGE_ABOVE);
    }

    return notes;
}

std::vector<BassNote> BassEngine::generatePedal(const ChordTones& tones, int barTicks, const BassEmotionProfile& profile, std::mt19937& rng) {
    std::vector<BassNote> notes;
    std::uniform_int_distribution<int> velDist(profile.velocityRange.first, profile.velocityRange.second);
    ...
    notes.push_back({tones.root, 0, duration, velDist(rng), BassArticulation::Sustained, "pedal"});

    return notes;
}

std::vector<BassNote> BassEngine::generateArpeggiated(const ChordTones& tones, int barTicks, const BassEmotionProfile& profile, std::mt19937& rng) {
    std::vector<BassNote> notes;
    std::uniform_int_distribution<int> velDist(profile.velocityRange.first, profile.velocityRange.second);

    std::vector<int> arpPitches = {tones.root, tones.third, tones.fifth, tones.third};
    int noteLen = barTicks / 4;
    constexpr int ARPEGGIATED_NOTE_TAIL_OFFSET = 10;
    int duration = static_cast<int>(noteLen * profile.noteLengthRatio) - ARPEGGIATED_NOTE_TAIL_OFFSET;

    std::vector<std::string> functions = {"root", "third", "fifth", "third"};
    for (size_t i = 0; i < arpPitches.size(); ++i) {
        notes.push_back({arpPitches[i], static_cast<int>(i * noteLen), duration, velDist(rng),
                        BassArticulation::Sustained, functions[static_cast<size_t>(i % functions.size())]});
    }

    return notes;
}

std::vector<BassNote> BassEngine::generateSyncopated(const ChordTones& tones, int barTicks, const BassEmotionProfile& profile, std::mt19937& rng) {
    std::vector<BassNote> notes;
    std::uniform_int_distribution<int> velDist(profile.velocityRange.first, profile.velocityRange.second);
    ...
    std::vector<int> positions = {0, eighth * SYNCOPATED_POSITION_1, eighth * SYNCOPATED_POSITION_2, eighth * SYNCOPATED_POSITION_3};
    std::vector<int> pitches = {tones.root, tones.root, tones.fifth, tones.root};

    for (size_t i = 0; i < positions.size(); ++i) {
        if (positions[i] < barTicks) {
            notes.push_back({pitches[i], positions[i], duration, velDist(rng), BassArticulation::Staccato, "synco"});
        }
    }
...
std::vector<BassNote> BassEngine::generateDriving(const ChordTones& tones, int barTicks, const BassEmotionProfile& profile, std::mt19937& rng) {
    std::vector<BassNote> notes;
    std::uniform_int_distribution<int> velDist(profile.velocityRange.first, profile.velocityRange.second);

    int eighth = TICKS_PER_BEAT / 2;
    int numNotes = barTicks / eighth;
    int duration = static_cast<int>(eighth * profile.noteLengthRatio) - MIN_NOTE_DURATION_OFFSET;

    for (int i = 0; i < numNotes; ++i) {
        int pitch = (i % 4 == 2) ? tones.fifth : tones.root;
        notes.push_back({pitch, i * eighth, duration, velDist(rng), BassArticulation::Staccato, "drive"});
    }

    return notes;
}

std::vector<BassNote> BassEngine::generatePulsing(const ChordTones& tones, int barTicks, const BassEmotionProfile& profile, std::mt19937& rng) {
    std::vector<BassNote> notes;
    std::uniform_int_distribution<int> velDist(profile.velocityRange.first, profile.velocityRange.second);
    ...
    for (int i = 0; i < barTicks / sixteenth; i += 2) {
        notes.push_back({tones.root, i * sixteenth, duration, velDist(rng), BassArticulation::Staccato, "pulse"});
    }

    return notes;
}

std::vector<BassNote> BassEngine::generateBreathing(const ChordTones& tones, int barTicks, const BassEmotionProfile& profile, std::mt19937& rng) {
    std::vector<BassNote> notes;
    std::uniform_int_distribution<int> velDist(profile.velocityRange.first, profile.velocityRange.second);
    ...
    int firstDuration = static_cast<int>(barTicks * BREATHING_FIRST_DURATION_RATIO);
    int secondStart = static_cast<int>(barTicks * BREATHING_SECOND_START_RATIO);
    int secondDuration = static_cast<int>(barTicks * BREATHING_SECOND_DURATION_RATIO);

    notes.push_back({tones.root, 0, firstDuration, velDist(rng), BassArticulation::Sustained, "root"});
    notes.push_back({tones.fifth, secondStart, secondDuration, velDist(rng) - VELOCITY_GHOST_REDUCTION, BassArticulation::Sustained, "fifth"});

    return notes;
}

BassOutput BassEngine::generate(
...
BassOutput BassEngine::generate(const BassConfig& config) {
    ...
    BassRegister reg = config.registerOverride.value_or(profile.preferredRegister);
    ...
    while (static_cast<int>(chords.size()) < config.bars) {
        for (size_t i = 0; i < config.chordProgression.size() && static_cast<int>(chords.size()) < config.bars; ++i) {
    ...
    for (int bar = 0; bar < config.bars; ++bar) {
        ChordTones tones = parseChord(chords[static_cast<size_t>(bar % static_cast<int>(chords.size()))], octave);
...
BassOutput BassEngine::generateForSection(
    const std::string& emotion,
    const std::vector<std::string>& chordProgression,
