# iDAW Knowledge Base - Upload to Custom GPT

This file should be uploaded as a "Knowledge" file in your Custom GPT configuration.

---

## USER'S SAMPLE LIBRARY (Google Drive - ~8GB)

Location: Google Drive

### Drum Packs
- **Drum Tornado 2023** (including Acoustic kit)
- **Drum Empire 2020** (+ add-ons)
- **Studio 2018 Percussion**
- **Studio 2009 Percussion**
- **Studio 2008 Percussion**

### Drum Elements
- **Kick** folders
- **Snare** folders (multiple)
- **HiHat** folders (multiple)
- **Percussion** folders (multiple)

### Synths & Keys
- **Synthwave** (multiple folders, organized by BPM: 100-120bpm, etc.)
- **Plucks & Keys**
- **Pads & Strings**
...
### Other
- **audio_vault** (main collection)
- **Sunny Perc**

### How to Use with iDAW Intent

When translating emotional intent to sample selection:

| iDAW Mood | Suggested Samples |
|-----------|-------------------|
| Grief, intimate | Acoustic drums (Drum Tornado), Pads & Strings |
| Defiance, power | Drum Empire, heavy kicks/snares |
| Nostalgia, warmth | Synthwave, Plucks & Keys |
| Anxiety, tension | Tight hats, sparse percussion |
| Lo-fi bedroom | Acoustic kit, light percussion, pads |

### Vernacular → Sample Suggestions

| Vernacular | Sample Direction |
|------------|------------------|
| "fat kick" | Drum Empire kicks, low-end heavy |
| "crispy hats" | HiHat folders, brighter samples |
| "warm pads" | Pads & Strings |
| "punchy snare" | Drum Tornado snares |
| "synthwave vibes" | Synthwave folder (match BPM) |
| "acoustic feel" | Drum Tornado Acoustic |
...
## THREE-WAY CRITIQUE SYSTEM

### Quality Checker (Technical)
Scores 0-100 on:
- **Timing naturalness:** Robotic (0) vs Human (100)
- **Velocity dynamics:** Static (0) vs Expressive (100)
- **Phrase continuity:** Choppy (0) vs Breathing (100)
- **Harmonic coherence:** Random (0) vs Intentional (100)

Issues to flag:
- Quantized-sounding patterns
- No velocity variation
- Missing breath gaps
- Voice leading violations (unless intentional)

### Interpretation Critic (Intent Matching)
Checks:
- **Emotion match:** Does music evoke stated emotion?
- **Cliché detection:** Is this the millionth I-V-vi-IV?
- **Vernacular misread:** Did "fat" get interpreted wrong?
- **Sample mismatch:** EDM samples for folk song?

Red flags:
- "Grief" but 130 BPM major key
- "Intimate" but stadium reverb
- "Lo-fi" but crystal clear mix

### Arbiter (Final Judgment)
Weighs both critics against artistic intent.

Verdicts:
- **PASS:** Ship it
- **REVISE:** Minor fixes needed (with specific suggestions)
- **RETHINK:** Fundamental mismatch, start over

Arbiter rules:
- Prevent over-correction (killing expression to fix "mistakes")
- Prevent under-correction (letting garbage through)
- Rule-breaks are allowed IF justified
- Imperfection serves lo-fi aesthetic
...
### 1. Interrogation Engine
- Phase 0: Core Wound (event, resistance, longing, stakes, transformation)
- Phase 1: Emotional Intent (mood, undertow, imagery, vulnerability)
- Parses vernacular input ("fat laid back grief")
- Outputs: EmotionalState

### 2. EmotionalState → MusicalParameters
- Maps emotion presets to technical parameters
- Applies modifiers (PTSD intrusion, misdirection, dissociation)
- Blends secondary emotions at 30% weight
- Outputs: MusicalParameters (tempo, mode, dissonance, timing feel, etc.)

### 3. Structure Generator
- Generates song sections based on emotional arc
- Templates: verse_chorus, slow_reveal, intimate
- Adjusts energy/density per section
- Handles misdirection reveal timing

### 4. Harmony Engine
- Generates chord progressions by emotion
- Applies rule breaks (modal interchange, non-resolution)
- Builds chords from scale degrees
- Supports borrowed chords

### 5. Melody Engine
- Generates notes preferring chord tones (70%) vs scale tones (30%)
- Applies register constraints
- Humanizes timing and velocity
- Respects space probability for rests

### 6. Groove Engine
- Drum patterns: basic, boom_bap, sparse, four_on_floor
- Applies swing to offbeats
- Humanizes timing per pocket feel (ahead/on/behind)
- Velocity accents on downbeats

### 7. MIDI Builder
- Assembles all tracks
- Supports AKAI MPK Mini live input
- Outputs standard MIDI file

### 8. Audio Tokenizer (optional)
- EnCodec-style encoding
- Converts audio ↔ tokens for transformer

### 9. Audio Generator (optional)
- MusicGen transformer
- Converts EmotionalState to text prompt
- Generates audio from prompt

### 10. Post-Processing
- Lo-fi degradation (bit crush, high roll-off, noise)
- Tape saturation (soft clipping, compression)
- Vinyl crackle (pops, noise floor)
- Applied based on imperfection_level

### 11. UI Layer
- Streamlit interface
- Three modes: Generate, Critique, Analyze
- Phase 0 + Phase 1 interrogation forms
...
### Usage
```python
from idaw_complete_pipeline import MPKMiniController

mpk = MPKMiniController()
mpk.connect()
mpk.start_recording()
# Play notes...
events = mpk.stop_recording()
```
...
iDAW-Music-Brain/
├── music_brain/
│   ├── core/           # EmotionalState, MusicalParameters, presets
│   ├── interrogation/  # InterrogationEngine
│   ├── structure/      # StructureGenerator
│   ├── harmony/        # HarmonyEngine, progressions
│   ├── melody/         # MelodyEngine
│   ├── groove/         # GrooveEngine, patterns
│   ├── midi/           # MIDIBuilder, MPK Mini
│   ├── audio/          # Tokenizer, Generator, PostProcessor
│   ├── analysis/       # Quality checker, Critic, Arbiter
│   └── data/           # JSON configs
├── ui/                 # Streamlit app
├── cli/                # Typer CLI
├── tests/
├── samples/            # Your sample library (gitignored)
