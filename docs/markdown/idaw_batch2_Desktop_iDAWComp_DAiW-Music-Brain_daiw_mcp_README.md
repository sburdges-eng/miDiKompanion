...
> **Auto-discovery:** any module placed in `daiw_mcp/tools/` with a
> `register_tools(server)` function is registered automatically. Add a new
> `*.py` file, implement the registration hook, and restart the MCP server—no
...
- `analyze_theory` - Complete music theory analysis
- `compare_audio_files` - Compare two audio files (BPM, key, feel)
- `batch_analyze_audio` - Batch analyze multiple audio files
- `export_audio_features` - Export comprehensive audio features
