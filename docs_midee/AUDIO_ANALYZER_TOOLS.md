...
Three new CLI tools have been created using the `AudioAnalyzer` class from `midee.audio.analyzer`. These tools extend DAiW's audio analysis capabilities for practical use cases.
...
### Dependencies
- `midee.audio.analyzer.AudioAnalyzer`
- `midee.audio.feel.compare_feel` (for comparison)
- `midee.audio.chord_detection.ChordDetector` (optional)
