...
- Run: ls -la midee/ 2>/dev/null || echo 'midee not found'

STEP 2: Create directories
- mkdir -p midee/models
- mkdir -p midee/data
...
FILE 1: midee/models/__init__.py
---
...
cp ~/Downloads/emotional_mapping.py midee/models/
cp ~/Downloads/daiw_knowledge_base.json midee/data/presets.json
cp ~/Downloads/rule_breaking_database.json midee/data/rule_breaks.json
```

OR have Claude Code fetch and create them:

```bash
claude "Create midee/models/emotional_mapping.py with the full EmotionalState and MusicalParameters dataclasses, EMOTIONAL_PRESETS dict, EMOTION_MODIFIERS, INTERVAL_EMOTIONS, and helper functions. Include:
...
```bash
claude "Update midee/__init__.py to import from models:

from midee.models import (
    EmotionalState,
    MusicalParameters,
    EMOTIONAL_PRESETS,
    get_parameters_for_state,
...
Add to __all__ list. Then run: python -c 'from midee import EmotionalState; print(EmotionalState)'"
```
...
cd DAiW-Music-Brain
python -c "
from midee.models import EmotionalState, EMOTIONAL_PRESETS, get_parameters_for_state
